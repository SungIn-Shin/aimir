...
*/
    ...
    */
     ...
     * @param tariffIndex: Tariff 醫낅쪟
     * @Description ?낅젰?쇱옄? 媛??媛源뚯슫 ?ㅼ쓬 tariff??applied date瑜?議고쉶.
     */
    public String getNextAppliedTariffDate(String date, Integer tariffIndex, Integer supplierId) {
		StringBuffer sb = new StringBuffer();
		sb.append("\n SELECT min(em.YYYYMMDD) as YYYYMMDD FROM (");
		sb.append("\n		SELECT DISTINCT YYYYMMDD FROM TARIFF_EM e LEFT OUTER JOIN TARIFFTYPE T ON(e.TARIFFTYPE_ID=t.ID)");
		sb.append("\n 		WHERE e.TARIFFTYPE_ID = :tariffIndex ");
		if(supplierId != null) {
     ...
     * @Date 2017.11.22.
     * @param date: 기준일
     * @param tariffIndex: 요금제 종류(residential, non-res, ...)
     * @param supplierId
     * @Description 입력일자 기준으로 적용할 Tariff 날짜를 반환한다. (Current Tariff)
     */
    public String getCurrentAppliedTariffDate(String date, Integer tariffIndex, Integer supplierId) {
    	StringBuffer sb = new StringBuffer();
		sb.append("\n SELECT max(em.YYYYMMDD) as YYYYMMDD FROM (");
		sb.append("\n		SELECT DISTINCT YYYYMMDD FROM TARIFF_EM e LEFT OUTER JOIN TARIFFTYPE T ON(e.TARIFFTYPE_ID=t.ID)");
		sb.append("\n 		WHERE e.TARIFFTYPE_ID = :tariffIndex ");
