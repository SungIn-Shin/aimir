...
//        for (int i = 0; i < yCodeLen && i < 4; i++) {
    ...
    public List<Object> getModemSearchGrid(Map<String, Object> condition){
        ...
        if(fwGadget.equals("Y")){
            ...
            if(!deviceIds.equals(""))
            	sbQuery.append("\nAND   mo.DEVICE_SERIAL IN ("+ deviceIds + ") ");
        }else{
        	if(!sModemId.equals("")){
...
//      return query.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
	...
	public List<Map<String,Object>> getSTSModem(Map<String,Object> condition) throws Exception {
		List<Map<String, Object>> result = null;
        String protocolType = (String)condition.get("protocolType");
        String modemType = (String)condition.get("modemType");
        String modelName = (String)condition.get("modelName");
        String meterTypeCode = (String)condition.get("meterTypeCode");
        String protocolVersion = (String)condition.get("protocolVersion");
        Integer tariffIndexId = condition.get("tariffIndexId") == null ? null : (Integer)condition.get("tariffIndexId");

        StringBuilder sb = new StringBuilder();

        sb.append("\nSELECT mo.deviceSerial AS DEVICESERIAL, me.mdsId AS MDSID, me.id AS METERID, c.contractNumber AS CONTRACTNUMBER, mo.phoneNumber AS PHONENUMBER ");
        sb.append("\nFROM MMIU mo, Meter me, Code co, DeviceModel model, Contract c ");
        sb.append("\nWHERE mo.id=me.modemId ");
        sb.append("\nAND me.id=c.meterId ");
        sb.append("\nAND me.meterTypeCodeId = co.id ");
        sb.append("\nAND co.code=:meterTypeCode ");
        sb.append("\nAND me.modelId=model.id ");
        sb.append("\nAND model.name=:modelName ");
        sb.append("\nAND mo.modemType=:modemType ");
        sb.append("\nAND mo.phoneNumber is not null ");
        sb.append("\nAND mo.protocolVersion = :protocolVersion and mo.protocolType=:protocolType");
        if(tariffIndexId != null) {
        	sb.append("\nAND c.tariffIndexId = :tariffIndexId");
        }
        

        Query query = getSession().createQuery(sb.toString());
        query.setString("meterTypeCode",meterTypeCode);
        query.setString("modelName",modelName);
        query.setString("modemType",modemType);
        query.setString("protocolVersion",protocolVersion);
        query.setString("protocolType",protocolType);
        if(tariffIndexId != null) {
        	query.setInteger("tariffIndexId", tariffIndexId);
        }
        result = query.setResultTransformer(Transformers.ALIAS_TO_ENTITY_MAP).list();
    ...
    /* SP-957 Clone On,Off */
    @Override
    public List<String> getDeviceList2(Map<String, Object> condition) {
    	String modelId = StringUtil.nullToBlank(condition.get("modelId"));
    	String versions = StringUtil.nullToBlank(condition.get("versions"));
    	String deviceId = StringUtil.nullToBlank(condition.get("deviceId"));
    	String fwVersion = StringUtil.nullToBlank(condition.get("fwVersion"));
    	...
    	if(!chkParent){ // fw 버전이 fwVersion인 것 모두 출력 (체크해제)
			...
			sbQuery.append("    select distinct(modem_id) from modem where  \n");
			sbQuery.append("        fw_ver is not null  \n");
    ...
    public List<Object> getModemList(Map<String, Object> condition){
    	...
    	String modemType = StringUtil.nullToBlank(condition.get("sModemType"));
    	String modelId = StringUtil.nullToBlank(condition.get("modelId"));
