...
create or replace PACKAGE BODY AIMIR_SLA AS
  ...
  PROCEDURE EXECUTE_SLARAWDATA AS
    cursor cursor_ip_mv_batches is
      select batch_id,batch_status,target_table,sla_date
      from ip_mv_batches
      where batch_status=3 and create_date > sysdate-1
      for update of batch_status, sla_date;
  ...
  BEGIN
    EXECUTE IMMEDIATE 'ALTER SESSION SET TIME_ZONE=''+00:00''';
    FOR row_batch IN cursor_ip_mv_batches
    LOOP
      IF row_batch.target_table = 'EM' THEN

        merge into sla_rawdata t
        using (select mdev_id,yyyymmdd,count(*) as lpcount from ip_mv_outbound_em where batch_id=row_batch.batch_id and channel=1 group by mdev_id,yyyymmdd) s
        ...
        begin
          insert into sla_stat_refresh(yyyymmdd) select distinct yyyymmdd from ip_mv_outbound_em where batch_id=row_batch.batch_id;
          exception
            when others then
              null;
        end;
        ...
        where deliver_date is null 
          and exists (select 1 from ip_mv_outbound_em d 
                      where d.mdev_type=a.mdev_type and d.mdev_id=a.mdev_id 
                        ...
                        and d.batch_id=row_batch.batch_id);

        delete from ip_mv_outbound_em where batch_id=row_batch.batch_id;

      ELSIF row_batch.target_table = 'WM' THEN

        merge into sla_rawdata t
        using (select mdev_id,yyyymmdd,count(*) as lpcount from ip_mv_outbound_wm where batch_id=row_batch.batch_id and channel=1 group by mdev_id,yyyymmdd) s
        on (t.meter_id=s.mdev_id and t.yyyymmdd=s.yyyymmdd)
        when matched then
          update set current_mv_count=current_mv_count+lpcount,last_update_date=current_date
        when not matched then
        ...
        begin
          insert into sla_stat_refresh(yyyymmdd) select distinct yyyymmdd from ip_mv_outbound_wm where batch_id=row_batch.batch_id;
          exception
            when others then
              null;
        end;
        ...
        set deliver_date = (select b.delivered_date 
                            from ip_mv_batches b
                            join ip_mv_outbound_wm c on b.batch_id=c.batch_id 
                            where c.mdev_type=a.mdev_type 
                              and c.mdev_id=a.mdev_id and c.channel=a.channel 
                              and c.yyyymmddhhmmss=a.yyyymmddhhmmss
                              and b.batch_id=row_batch.batch_id)
        ...
        delete from ip_mv_outbound_wm where batch_id=row_batch.batch_id;

      ELSIF row_batch.target_table = 'GM' THEN

        merge into sla_rawdata t
        using (select mdev_id,yyyymmdd,count(*) as lpcount from ip_mv_outbound_gm where batch_id=row_batch.batch_id and channel=1 group by mdev_id,yyyymmdd) s
        on (t.meter_id=s.mdev_id and t.yyyymmdd=s.yyyymmdd)
        when matched then
          update set current_mv_count=current_mv_count+lpcount,last_update_date=current_date
        when not matched then
        ...
        begin
          insert into sla_stat_refresh(yyyymmdd) select distinct yyyymmdd from ip_mv_outbound_gm where batch_id=row_batch.batch_id;
          exception
            when others then
              null;
        end;
        ...
        set deliver_date = (select b.delivered_date 
                            from ip_mv_batches b
                            join ip_mv_outbound_gm c on b.batch_id=c.batch_id 
                            where c.mdev_type=a.mdev_type 
                              and c.mdev_id=a.mdev_id and c.channel=a.channel 
                              and c.yyyymmddhhmmss=a.yyyymmddhhmmss
                              and b.batch_id=row_batch.batch_id)
        ...
        delete from ip_mv_outbound_gm where batch_id=row_batch.batch_id;

      ELSIF row_batch.target_table = 'HM' THEN

        merge into sla_rawdata t
        using (select mdev_id,yyyymmdd,count(*) as lpcount from ip_mv_outbound_hm where batch_id=row_batch.batch_id and channel=1 group by mdev_id,yyyymmdd) s
        on (t.meter_id=s.mdev_id and t.yyyymmdd=s.yyyymmdd)
        when matched then
          update set current_mv_count=current_mv_count+lpcount,last_update_date=current_date
        when not matched then
        ...
        begin
          insert into sla_stat_refresh(yyyymmdd) select distinct yyyymmdd from ip_mv_outbound_hm where batch_id=row_batch.batch_id;
          exception
            when others then
              null;
        end;
        ...
        set deliver_date = (select b.delivered_date 
                            from ip_mv_batches b
                            join ip_mv_outbound_hm c on b.batch_id=c.batch_id 
                            where c.mdev_type=a.mdev_type 
                              and c.mdev_id=a.mdev_id and c.channel=a.channel 
                              and c.yyyymmddhhmmss=a.yyyymmddhhmmss
                              and b.batch_id=row_batch.batch_id)
        ...
        delete from ip_mv_outbound_hm where batch_id=row_batch.batch_id;
      END IF;
      ...
      update ip_mv_batches set batch_status=4,sla_date=current_date where current of cursor_ip_mv_batches;
    END LOOP;
...
begin
    ...
    DBMS_SCHEDULER.CREATE_JOB (
       ...
       repeat_interval      => 'FREQ=MINUTELY;INTERVAL=10;BYSECOND=17', 
       enabled              =>  TRUE,
