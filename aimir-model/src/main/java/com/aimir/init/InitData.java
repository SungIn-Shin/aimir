...
public class InitData {
        ...
        try {
            ...
            for (int i = 0; i < list.size(); i++) {
                ...
                while (itableIt.next()) {
                    ...
                    log.info("====================================================================================");
                    log.info("INIT Data Table[" + tableName + "] Column["
                            + columnName.toString() + "] Count[" + rowCount
                            + "]");
                    int r = 0;
                    try {
                    ...
                    log.info("END Data Table[" + tableName + "] Count["
                            + (r + 1) + "]");
                }
            }
        }
    ...
    private void save(int row, String tableName, Column[] columns,
            ITable itable, String lang) throws Exception {
        ...
        try {
            for (int c = 0; c < columns.length; c++) {
                ...
                log.info("Table[" + tableName + "] Column[" + columnName
                        + "] Value[" + val + "]");
                ...
                if (orm.checkRelationOrObject(columnName)) {
                    ...
                    Class<?> expectedType = getParameterTypes(searchName,
                            assInstance);

                    if (val != null) {
                        val = TypeCast.stringTocast(expectedType,
                                val.toString());
                    }

                    if (searchName != null && !"".equals(searchName)) {
                        Object[] param = new Object[] { searchName, val };
                        assTarget = InvokeUtil.daoGetter(ctx,
                                refClassName.toLowerCase(), param,
                                "findbycondition");
                    }

                    if (assTarget != null) {
                        InvokeUtil.objectSetter(target, assTarget, method,
                                columnName);
                    }

                } else {
                    InvokeUtil.objectSetter(ctx, orm, target, val, method,
                            columnName);
                }
            }
            ...
            e.printStackTrace();
            throw new Exception("Table[" + tableName + "] Column[" + columnName
                    + "] Value[" + val + "]", e.getCause());
        }
    }
    ...
    public Class<?> getParameterTypes(String attrName, Class<?> checkClass) {
        ...
        try {
            ...
            if (checkClass.getSuperclass() == null) {
                ...
                final Class<?> superfieldType = getParameterTypes(attrName,
                        checkClass.getSuperclass());
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            if (args.length == 1) {
                ...
                log.info(
                        "Usage : mvn antrun:run -Dfile.encoding=CHAR_SET -DdataFile=Directory or Filename");
                System.exit(1);
            }
        } catch (Exception ex) {
