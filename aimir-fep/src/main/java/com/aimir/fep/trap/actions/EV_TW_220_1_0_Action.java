 ...
 */
@Service
public class EV_TW_220_1_0_Action implements EV_Action {
	private static Log log = LogFactory.getLog(EV_TW_220_1_0_Action.class);
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		if (meter == null) {
			...
			if (meterModel == null || "".equals(meterModel)) {
				meterModel = "NRAM-1405DR60"; //default waison meter
			}else {
				meterModel = getMeterModel(meterModel);
			}
			...
			if (model != null) {
				...
				log.debug("[HSW] HHH setModel:"+model.getName());
			}
			if (modem != null) {
		...
		if (modem != null) {
			//가나 tvws split 미터는 한개의 모뎀에 다수가 연결할 수 있다.
			if(meter.getModel() != null && !meter.getModel().getName().equals("NRDT-1205DR60")) {
...
//				modemDao.update_requires_new(modem);
				...
				if (orgMeter != null) {
					eventUtil.sendEvent("Equipment Replacement", 
					        ...
					        modem.getDeviceSerial(), new String[][] { { "equipType", meter.getMeterType().getName() },
					    { "oldEquipID", orgMeter.getMdsId() }, { "newEquipID", meter.getMdsId() } });
				}
			} else if (!orgModem.getDeviceSerial().equals(modem.getDeviceSerial())) {
				
				if(meter.getModel() != null && !meter.getModel().getName().equals("NRDT-1205DR60")) {
					//tvws split 미터의 경우 해당 미터의 모뎀만 변경한다.
            		meter.setModem(modem);
            		meterDao.update(meter);
				} else {
...
//						meterDao.updateModemIdNull(m);
				...
				eventUtil.sendEvent("Equipment Replacement",
				    ...
				    { "oldEquipID", orgModem.getDeviceSerial() }, { "newEquipID", modem.getDeviceSerial() } });
			}
			// DELETE START SP-917
//			meter.setModem(modem);
//			meterDao.update_requires_new(meter);
    ...
    public String getMeterModel(String meterModel) {

    	if(meterModel.equals("WS34-201-W5")) {
    		return "NRAM-3405CT10";
    	}
    	else if(meterModel.equals("WS34-210-W5")) {
    		return "NRAM-3410DR100";
    	}
    	else if(meterModel.startsWith("WS14")) {
    		return "NRAM-1405DR60";
    	}
    	else if(meterModel.startsWith("WS12")) {
    		return "NRDT-1205DR60";
    	}
