...
public class EV_GG_200_2_0_Action implements EV_Action
{
    ...
    public void execute(FMPTrap trap, EventAlertLog event) throws Exception
    {
        ...
        /*
         * GPRS모뎀이 NTP시간동기화 실패후 2051년으로 올라오는 것을 현재시간으로 변경한다.
         */
        if (sEventTime.startsWith("2051")) {
            sEventTime = DateTimeUtil.getDateString(new Date());
            event.setOpenTime(sEventTime);
            
            if (event.getCloseTime() != null && event.getCloseTime().startsWith("2051")) {
                event.setCloseTime(sEventTime);
            }
        }
        ...
        try {
            ...
            if(event.getEventAttrValue("evtRelayStatus") != null && !"".equals(event.getEventAttrValue("evtRelayStatus"))){
                ...
                Code currentStatus = meter.getMeterStatus();
                if (currentStatus != null && 
                        ((relayOn && currentStatus.getName().equals("CutOff")) ||
                                ((!relayOn && currentStatus.getName().equals("Normal"))))) {
                    DeviceConfig config = null;
                    if (meter.getModel() != null)
                        config = meter.getModel().getDeviceConfig();
                    
                    log.debug("Saver[" + config.getSaverName() + "]");

                    AbstractMDSaver saver = null;
                    if (config.getSaverName() != null && !"".equals(config.getSaverName()))
                        saver = (AbstractMDSaver)DataUtil.getBean(Class.forName(config.getSaverName()));
                    
                    if (relayOn) saver.updateMeterStatusNormal(meter);
                    else saver.updateMeterStatusCutOff(meter);
                    
                    procRelayEvent(true, relayStatusMsg, sEventTime, event); 
                }
                else {
                    procRelayEvent(false, relayStatusMsg, sEventTime, event); 
                }
            }
        }
        finally {
    ...
    public void procRelayEvent(boolean isStatusChanged, String relayStatus, String sEventTime, EventAlertLog event){

        log.debug("RelayEvent Event Time[" + sEventTime + "] Name[" + relayStatus +"] isStatusChanged="+isStatusChanged+"]");        
        
        if(isStatusChanged && (relayStatus.equals("Relays disconnected by command")
        		|| relayStatus.equals("Relays connected")
                ||relayStatus.equals("Relays disconnected by push button"))) {
            EventAlert eventAlert = eaDao.findByCondition("name", "EnergyLevelChanged");
            event.setEventAlert(eventAlert);
            ea = EventUtil.makeEventAlertAttr("message", "java.lang.String", relayStatus);
            event.append(ea);
            if(relayStatus.equals("Relays connected")) {
                event.setStatus(EventStatus.Cleared);
            }
        }else if(!isStatusChanged && (relayStatus.equals("Relays disconnected by command")
        		|| relayStatus.equals("Relays connected")
                ||relayStatus.equals("Relays disconnected by push button"))) {
        ...
        if(relayStatus.equals("Not yet Read")) {
            EventAlert eventAlert = eaDao.findByCondition("name", "Equipment Notification");
            event.setEventAlert(eventAlert);
            ea = EventUtil.makeEventAlertAttr("message", "java.lang.String", relayStatus);
            event.append(ea);
        ...
        if(relayStatus.equals("Relays released for reconnection")){
            EventAlert eventAlert = eaDao.findByCondition("name", "EnergyLevelChanged");
            event.setEventAlert(eventAlert);
            ea = EventUtil.makeEventAlertAttr("message", "java.lang.String", relayStatus);
            event.append(ea);
