...
//import com.aimir.dao.device.GprsStatusLogDao;
...
//import com.aimir.model.device.GprsStatusLog;
...
//    @Autowired
//    GprsStatusLogDao gprsStatusLogDao;
    ...
    public void execute(FMPTrap trap, EventAlertLog event) throws Exception
    {
        ...
        try {
            ...
            if(installedDate == null || "".equals(installedDate) ||!installedDate.subSequence(0, 4).equals(TimeUtil.getCurrentTimeMilli().substring(0, 4))){
            	...
            	commDate = currentTime;
            	event.setOpenTime(currentTime);
            ...
            String gprsIMEI = event.getEventAttrValue("evtGprsIMEI");     
            String simIMSI = event.getEventAttrValue("evtSimIMSI");
            String simIccId = event.getEventAttrValue("evtSimICCID");
            ...
            /**             
             * 2018-05-21 TVWS에서 Gprs_Status사용여부를 알수없어서 주석처리
             *          	
         	String gprsStatus = event.getEventAttrValue("evtGPRSStatus");
            int rssi = 0;
            int ber = 0;
                       
            if (gprsStatus != null && !"".equals(gprsStatus)) {
            	log.info("GPRS Status[" + gprsStatus + "]");
            	byte[] _gprsStatus = Hex.encode(gprsStatus);
            	GprsStatusLog gsLog = new GprsStatusLog();
            	gsLog.setModemId(trap.getSourceId());
            	gsLog.setStatusTime(installedDate);
            	rssi = DataUtil.getIntToByte(_gprsStatus[0]);
            	ber = DataUtil.getIntToByte(_gprsStatus[1]);
            	gsLog.setRssi(rssi);
            	gsLog.setBer(ber);
            	gsLog.setStatus(_gprsStatus);
            	
            	gprsStatusLogDao.add(gsLog);
            } */
         ...
         // get modem
            ...
            if (modem == null) {
                ...
                modem.setSimNumber(simIMSI);
                modem.setIccId(simIccId);
