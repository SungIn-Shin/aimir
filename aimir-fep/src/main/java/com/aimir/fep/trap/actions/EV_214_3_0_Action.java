...
public class EV_214_3_0_Action implements EV_Action
{
    ...
    public void execute(FMPTrap trap, EventAlertLog event) throws Exception
    {
        ...
        try {
            ...
            fwUtil.updateTriggerHistory(triggerId,step.getCode(),triggerState);
            if(equipKind!=null && equipKind.equals("MCU")){
        		
        		if (state.equals(FW_STATE.Success)) {
        			fwUtil.updateOTAHistory(triggerId, mcuId, FW_OTA.All,FW_STATE.Success, "");
        		} else if (state.equals(FW_STATE.Fail)) {
        			fwUtil.updateOTAHistory(triggerId, mcuId, FW_OTA.All,FW_STATE.Fail, "");
        		} else if (state.equals(FW_STATE.Cancel)) {
        			fwUtil.updateOTAHistory(triggerId, mcuId, FW_OTA.All,FW_STATE.Unknown, "");
        		} 
            }
		...
		for (int i = 0; i < sie.length; i++) {
			...
			if (sie[i].getSensorID() != null && sie[i].getSensorOTAState() != null) {
				...
				if ((otaState & FW_OTA.All.getStep()) == FW_OTA.All.getStep()) {
					fwUtil.updateOTAHistory(triggerId, sie[i].getSensorID().toString(), FW_OTA.All,FW_STATE.Unknown, "");
				} else if ((otaState & FW_OTA.Scan.getStep()) == FW_OTA.Scan.getStep()) {
					fwUtil.updateOTAHistory(triggerId, sie[i].getSensorID().toString(), FW_OTA.Scan,FW_STATE.Unknown, "");
				} else if ((otaState & FW_OTA.Install.getStep()) == FW_OTA.Install.getStep()) {
					fwUtil.updateOTAHistory(triggerId, sie[i].getSensorID().toString(), FW_OTA.Install,FW_STATE.Unknown, "");
				} else if ((otaState & FW_OTA.Verify.getStep()) == FW_OTA.Verify.getStep()) {
					fwUtil.updateOTAHistory(triggerId, sie[i].getSensorID().toString(), FW_OTA.Verify,FW_STATE.Unknown, "");
				} else if ((otaState & FW_OTA.DataSend.getStep()) == FW_OTA.DataSend.getStep()) {
					fwUtil.updateOTAHistory(triggerId, sie[i].getSensorID().toString(), FW_OTA.DataSend,FW_STATE.Unknown, "");
				} else if ((otaState & FW_OTA.Check.getStep()) == FW_OTA.Check.getStep()) {
					fwUtil.updateOTAHistory(triggerId, sie[i].getSensorID().toString(), FW_OTA.Check,FW_STATE.Unknown, "");
				} else if ((otaState & FW_OTA.Init.getStep()) == FW_OTA.Init.getStep()) {
					fwUtil.updateOTAHistory(triggerId, sie[i].getSensorID().toString(), FW_OTA.Init,FW_STATE.Unknown, "");
				}
			}
