...
public class EV_TW_240_2_0_Action implements EV_Action {
    ...
    public void makeModemEvent(Modem modem, Meter meter, String issueDate, Integer eventIdN, String payload) throws Exception {
        ...
        if (alarmId == AlarmEvent.AlarmId.PowerFail) {
            ...
            log.debug("[MODEM_OTA] Equipment_FirmwareUpdate received. ModemId=" + modem.getDeviceSerial() + ", MeterId=" + meter.getMdsId() + ", Modem FW Version old[" + modem.getFwVer() + "] to new[" +ver+ "]");
            modem.setFwVer(ver);
            modemDao.update(modem);
            ...
            fwUtil.updateOTAHistory(modem.getDeviceSerial(), DeviceType.Modem, openTime, histMsg, EVENT_MESSAGE);
            ...
            log.debug("[METER_OTA_BY_MODEM] MeterUpgradeFinished Event received. ModemId=" + modem.getDeviceSerial() + ", MeterId=" + meter.getMdsId() + ", NameSpace=" + modem.getNameSpace() 
                    + ", Modem FW Version old[" + meter.getSwVersion() + "] to new[" +fwVersion+ "]");
            
            if(modem != null  && "TW".equals(modem.getNameSpace())) {
                String HexVersion = (fwVersion.length() > 4) ? (fwVersion.substring(fwVersion.length() - 4, fwVersion.length())) : fwVersion;
                
                Map<String,Object> condition = new HashMap<String,Object>();
                condition.put("ni_fwVer", HexVersion);
                condition.put("equip_kind", "meter");
                
                List<Object> resultList = firmwareDao.getFirmwareFileList(condition);
                if(resultList != null && resultList.size() > 0) {
                    Object[] resultData = (Object[]) resultList.get(0);
                    String otaVersion = (String) resultData[8];
                    
                    if(meter != null && meter.getSwVersion().contains("___")) {
                        String[] array = meter.getSwVersion().split("___");
                        fwVersion =  array[0] + "___" + otaVersion;
            ...
            fwUtil.updateOTAHistory(meter.getMdsId(), DeviceType.Meter, openTime, message, EVENT_MESSAGE);
            ...
            eventClassName = "Meter Value Alarm";
            message = "Metering Value Incorrect";
        } else if(alarmId == AlarmEvent.AlarmId.DoorOpen) {
            eventClassName = "Cover Alarm";
            message = "Door Open";
        } else if(alarmId == AlarmEvent.AlarmId.DoorClose) {
            eventClassName = "Cover Alarm";
            message = "Door Close";
        } else if(alarmId == AlarmEvent.AlarmId.CircuitBreaker) {
            eventClassName = "EnergyLevelChanged";
            if (bx[3] == 0x00)
                message = "CB(Circuit Breaker) Status OFF";
            else if (bx[3] == 0x01)
                message = "CB(Circuit Breaker) Status ON";
        } else {
        ...
        eventUtil.sendEvent(eventClassName, TargetClass.valueOf(activatorType), 
                activatorId, issueDate, new String[][] { { "message", message } }, _event);
    ...
    public void makeModemEvent(String modemId, String issueDate, Integer eventIdN, String payload) {
        try {
            ...
            if("TW".equals(modem.getNameSpace()) && "NAMR-P20DSR".equals(modem.getModel().getName())) { //Ghana TVWS - Split Modem
                //모뎀에 해당 미터들이 모두 업그레이드 되었을 때만 이벤트 전송하기로 함 - 2018.07.12
                for(Meter m : meterList) {
                    makeModemEvent(modem, m, issueDate, eventIdN, payload);
                }
            } else {
                makeModemEvent(modem, meterList.get(0), issueDate, eventIdN, payload);  
            }               
        } catch (Exception e) {
            log.error(e, e);
        }
