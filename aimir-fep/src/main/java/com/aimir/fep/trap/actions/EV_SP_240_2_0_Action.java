...
public class EV_SP_240_2_0_Action implements EV_Action {
	...
	public void execute(FMPTrap trap, EventAlertLog event) throws Exception {
		...
		log.debug("EV_SP_240_2_0_Action : EventName[modemTamper] " + " EventCode[" + trap.getCode() + 
		        "] MCU[" + trap.getMcuId() + "] TargetClass[" + event.getActivatorType() + "] openTime[" + openTime + "]");
	...
	public void makeModemEvent(Modem modem, Meter meter, String issueDate, Integer eventIdN, String payload) throws Exception {
		...
		if (alarmId == AlarmEvent.AlarmId.PowerFail) {
			...
			log.debug("[METER_OTA_BY_MODEM] MeterUpgradeFinished Event received. ModemId=" + modem.getDeviceSerial() + ", MeterId=" + meter.getMdsId() + ", Modem FW Version old[" + meter.getSwVersion() + "] to new[" +fwVersion+ "]");
		...
		eventUtil.sendEvent(eventClassName, TargetClass.valueOf(activatorType), activatorId, 
		        issueDate, new String[][] { { "message", message } }, _event);
	...
	public void updateOTAHistory(String deviceId, DeviceType deviceType, String openTime, String resultStatus) {
		...
		log.debug("Update OTA History param save. DeviceId=" + deviceId + ", DeviceType=" + deviceType.name() + ", OpentTime=" + openTime + ", ResultStatus=" + resultStatus);
