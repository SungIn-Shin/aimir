...
import com.aimir.fep.bypass.decofactory.consts.HdlcConstants.HdlcMeterLevel;
 ...
 *
 */
public class NestedHDLCDecoratorForECG extends NestFrameDecorator {
	private static Logger logger = LoggerFactory.getLogger(NestedHDLCDecoratorForECG.class);
	...
	private boolean rrCommand = false;
	
	private final int METER_LEVEL = 3;
	 ...
	 * @param nestedFrame
	 */
	public NestedHDLCDecoratorForECG(INestedFrame nestedFrame) {
		super(nestedFrame);
	}
	...
	public byte[] encodeOfAddress(HdlcAddressType type) {
		...
		*/
		...
		switch (type) {
		case DEST:
			// 임시로 고정된값 세팅 : 0x00020021
			result = new byte[] { (byte) 0x00, (byte) 0x02, (byte) 0x00, (byte) 0x21 };  
			break;
		case SRC:
			// 임시로 고정된값 세팅 : 0x03
	...
	public byte[] encodeofAddresDESC(String meterId) {
		byte[] result = new byte[] {(byte) 0x00, (byte) 0x02, (byte) 0x00};
		String desc = null;
		if("0024000000018".equals(meterId) || "0024000000091".equals(meterId)) {
			desc = meterId.substring(meterId.length() - 2, meterId.length());
		} else {
			desc = meterId.substring(meterId.length() - 3, meterId.length() - 1);
		}
		
		byte[] hexAddr = DataUtil.get2ByteToInt(desc);
		int num = (((hexAddr[1] & 0xff) << 1) + 1);
		
		byte[] hexDesc = Hex.encode(Integer.toHexString(num));
		result = DataUtil.append(result, hexDesc);
		return result;
	}
	
	public byte[] encodeOfAddressSRC(HdlcMeterLevel level) {		
		byte[] result = null;
		switch (level) {
		case LEVEL1:
			// 임시로 고정된값 세팅 : 3
			result = new byte[] { (byte) 0x07 };
			//result = new byte[] { (byte) 0x03 };
			break;
		case LEVEL2:
			// 임시로 고정된값 세팅 : 102
			result = new byte[] { (byte) 0xCD };
			//result = new byte[] { (byte) 0x66 };
			break;		
		case LEVEL3: // level 2
			// 임시로 고정된값 세팅 : 1
			result = new byte[] { (byte) 0x33, (byte) 0x33, (byte) 0x33, (byte) 0x33, (byte) 0x33, (byte) 0x33, (byte) 0x33, (byte) 0x33 };
			//result = new byte[] { (byte) 0x01 };
			break;
		case PUBLIC: // level 3
			// 임시로 고정된값 세팅 : 16
			result = new byte[] { (byte) 0x21 };
			//result = new byte[] { (byte) 0x10 };
			break;
		}
	...
	public byte[] encode(HdlcObjectType hdlcType, Procedure procedure, HashMap<String, Object> param, String command) {
		logger.info("## Excute NestedHDLCDecorator Encoding [{}] | isHLS[{}]", hdlcType.name(), isProtocalHLS());
		...
		try {
			...
			if(getMeterId() !=  null) {
				if(isProtocalHLS()) 
					...
					gdDLMSFrame = DataUtil.append(gdDLMSFrame, encodeofAddresDESC(getMeterId()));
			...
			if (hdlcType == HdlcObjectType.SNRM) {
				...
				if(rrCommand) {
					byte[] control = new byte[] { HdlcConstants.getRSCount(receciveCount) };
	...
	public boolean decode(byte[] frame, Procedure procedure, String command) {
		...
		try {
			...
			if(isProtocalHLS()) {
				if((frameType[0] & 0xA8) == 0xA8) //tvws 암호화 및 rr 프레임 작업
					rrCommand = true;
				else 
					rrCommand = false;
			}
