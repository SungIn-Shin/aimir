...
public class HdlcConstants {
	...
	public enum HdlcMeterLevel {
		LEVEL1(1), LEVEL2(2), LEVEL3(3), PUBLIC(9);
		
		private int type;

		private HdlcMeterLevel(int type) {
			this.type = (byte) type;
		}

		public static HdlcMeterLevel getItem(int value) {
			for (HdlcMeterLevel fc : HdlcMeterLevel.values()) {
		...
		public static int getItem(HdlcMeterLevel value) {
			for (HdlcMeterLevel fc : HdlcMeterLevel.values()) {
...
/*		int tempA = receciveCount;
	...
	public static byte getRSCount(int receciveCount) {
		String binaryStringA = String.format("%03d", new BigInteger(Integer.toBinaryString(receciveCount))) + "1";
		String binaryStringB = "0001";
