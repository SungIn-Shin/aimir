 ...
 *
 */
public class NestedHDLCDecoratorForMOE extends NestFrameDecorator {
	private static Logger logger = LoggerFactory.getLogger(NestedHDLCDecoratorForMOE.class);
	 ...
	 * @param nestedFrame
	 */
	public NestedHDLCDecoratorForMOE(INestedFrame nestedFrame) {
		super(nestedFrame);
	}
	...
	public byte[] encode(HdlcObjectType hdlcType, Procedure procedure, HashMap<String, Object> param, String command) {
		...
		try {
			...
			if (hdlcType == HdlcObjectType.SNRM) {
				...
				gdDLMSFrame = DataUtil.append(gdDLMSFrame, new byte[] { HdlcConstants.getRSCount(receciveCount, sendCount) });
				receciveCount++;
				sendCount++;
	...
	public boolean decode(byte[] frame, Procedure procedure, String command) {
		...
		try {
			...
			if (control[0] == 0x73) { // UA
				...
				setType(0); // 초기화
				logger.debug("[HDLC] R:S = [{}]{}", control[0], HdlcConstants.getRSCount(control[0]));
			}
			...
			if(3 < (frame.length - pos)){
				...
				// 10 : frameType + frameLength + arcAddress + descAddress + Control + hcs + fcs
