...
public class EventUtil
{
    ...
    public EventAlertLog makeEventAlertLog(String eventAlertName, Object inst, EventAlertAttr[] attrs) throws Exception {
        try {
    ...
    public void sendEvent( String eventAlertName, 
	...
	public void sendEvent(String eventAlertName, Object inst, EventAlertAttr[] attrs, String timestamp) throws Exception {
		EventAlert eventRule = findRule(eventAlertName);
		if (eventRule == null) {
	...
	@Transactional(propagation=Propagation.REQUIRES_NEW)
	public void sendEvent(String eventAlertName, String activatorType, String activatorId, int supplierId) {
		try {
    ...
    private EventAlert findRule(String eventAlertName) throws Exception 
    {
        log.debug("EventAlertName[" + eventAlertName + "]");
        
        return eaDao.findByCondition("name", eventAlertName);
     ...
     */
    public void sendEvent(String eventAlertName, TargetClass activatorType,
    ...
    public void sendEvent(String eventAlertName, TargetClass activatorType,
                                 ...
                                 String[][] attrs, EventAlertLog event) {
        try {
            event.setActivatorId(activatorId);
            event.setActivatorType(activatorType);
            
            if(activatorType == TargetClass.DCU){
                MCU mcu = mcuDao.get(activatorId);
                if(mcu != null){
                    event.setActivatorIp(mcu.getIpAddr());
                    event.setLocation(mcu.getLocation());
                    event.setSupplier(mcu.getSupplier());
                ...
                Modem modem = modemDao.get(activatorId);
                if(modem != null){
                    event.setActivatorIp(modem.getIpAddr());
                    event.setSupplier(modem.getSupplier());
                ...
                Meter meter = meterDao.get(activatorId);
                if(meter != null){
            ...
            event.setWriteTime(DateTimeUtil.getDST(null,timestamp));
    ...
    @Transactional(propagation=Propagation.REQUIRES_NEW)
    public String sendEvent(long logId, EventStatus eventStatus) {
