		...
		case UpgradeStartRequest:
			...
			if ((fwVersion != null) && (fwVersion.length() > 2)) {
				...
				if(0 < fwVersion.indexOf(".")){ // DCU or Modem
					...
					//String version = fwVersion.substring(fwVersion.length() - 4, fwVersion.length()); //Soria
					//TVWS Meter FW Version
					int yyTag = Integer.parseInt(seqNumber.substring(0, 2)) * 400;
					int mmTag = Integer.parseInt(seqNumber.substring(2, 4)) * 32;
					int ddTag = Integer.parseInt(seqNumber.substring(4, 6));
					String version = String.valueOf(yyTag + mmTag + ddTag);
					
					//convertVer = DataUtil.readByteString(version); //SORIA
					convertVer = DataUtil.get2ByteToInt(Integer.parseInt(version)); //GHANA-TVWS
				}
				
				log.debug("Attribute2. FW Version = " + fwVersion + ", HEX Converted version = " + Hex.decode(convertVer));
				out.write(convertVer);
			}
			...
			if ((fwModel != null) && (fwModel.length()>0)) {
				...
				log.debug("Attribute3. FW Model Name = " + fwModel + ", HEX Converted fwModel = " + Hex.decode(b));  // 미터의 경우는 펌웨어파일명으로 들어가게됨.
				out.write(b);
			}			
			// INSERT END SP-681
