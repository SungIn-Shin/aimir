...
//    private Hashtable<Long, IoBuffer> response = new Hashtable<Long, IoBuffer>();
    ...
    public void setIsCmdAdapter(boolean isCmdAdapter) {
        this.isCmdAdapter = isCmdAdapter;
    }
    ...
    public void exceptionCaught(IoSession session, Throwable cause)
        ...
        try {
        	...
        	log.debug("HandlerName=[" + getHandlerName() + "][code trace] sessionId=" + session.getId() + ", exceptionCaught call1=" + session.getRemoteAddress());
			//SP-890
    ...
    public void messageReceived(IoSession session, Object message) throws Exception
    {
        ...
        if (message instanceof IoBuffer) {
           ...
           // log.debug("[messageReceived]"+Hex.decode(((IoBuffer)message).array()));
            ...
            validateModem(nodeId, gframe.getNetworkStatus(), ipAddr);
            
        }
    }  //~messageReceived
    
    private void validateModem(String nodeId, NetworkStatus ns, String ipAddr) {
    ...
    private void makeModemEvent(String nodeId, AlarmEvent alarmEvt) {
        try {
            for (int i = 0; i < alarmEvt.getCount(); i++) {
                DataUtil.getBean(EV_SP_240_2_0_Action.class).makeModemEvent(nodeId, alarmEvt.getTime()[i], 
    ...
    private void makeMeterEvent(String nodeId, MeterEvent meterEvt) throws Exception {
        try {
            for (MeterEventFrame mef : meterEvt.getMeterEventFrame()) {
                DataUtil.getBean(EV_SP_220_2_0_Action.class).doAlarm(nodeId, mef.getValue());
            }
        }
        catch (Exception e) {
            log.error(e, e);
    ...
    private void doTrap(String modemId, String ipAddr, Command cmd) throws Exception {
        ...
        try {
            ...
            OndemandType mode = OndemandType.DirectMode; //set default directmode(gprs)
