...
//					throw new Exception("Can't found Coordinator.");
			...
			if (versionCheck){
				...
				case METER_RF_BY_THIRD_PARTY_COORDINATOR:
					...
					resultTable = gw.cmdReqImagePropagate(mcuSysId, otaType.getTypeCode(), 0, params.get("image_identifier").toString(), params.get("fw_path").toString(), params.get("checkSum").toString(), newFwVersion, (String) params.get("image_identifier"), cloneCount, null);
					logger.debug("Result = " + resultTable.toString());
				...
				case MODEM_RF_BY_THIRD_PARTY_COORDINATOR:
					...
					resultTable = gw.cmdReqImagePropagate(mcuSysId, otaType.getTypeCode(), 0, params.get("image_identifier").toString(), params.get("fw_path").toString(), params.get("checkSum").toString(), newFwVersion, targetModel, cloneCount, null); // "NAMR-P214SR" for use third party RF Modem ota
					logger.debug("Result = " + resultTable.toString());
		...
		if (!executeResult) {
			...
			if (otaType == OTAType.METER_RF_BY_THIRD_PARTY_COORDINATOR) {
				...
				action66.makeEvent(TargetClass.DCU, mcuSysId, TargetClass.EnergyMeter, openTime, resultCode, errMsg, "HES");
				...
				action66.makeEvent(TargetClass.DCU, mcuSysId, TargetClass.SubGiga, openTime, resultCode, errMsg, "HES");
				...
				for (String targetId : filterValue) {
					action66.makeEvent(TargetClass.DCU, targetId, TargetClass.DCU, openTime, resultCode, errMsg, "HES");
