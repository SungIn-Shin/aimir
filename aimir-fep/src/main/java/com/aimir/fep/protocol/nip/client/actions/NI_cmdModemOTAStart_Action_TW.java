...
public class NI_cmdModemOTAStart_Action_TW extends NICommandAction {
    private static Logger logger = LoggerFactory.getLogger(NI_cmdModemOTAStart_Action_TW.class);
    ...
    private String actionTitle = "NI_cmdModemOTAStart_Action_TW";
    ...
    public NI_cmdModemOTAStart_Action_TW() {
        ...
        if (Boolean.parseBoolean(FMPProperty.getProperty("soria.protocol.modem.rf.dtls.use", "false"))) {
            ...
            SEND_IAMGE_RETRY_TIMEOUT = Integer.parseInt(FMPProperty.getProperty("ota.firmware.modem.datasend.retry.timeout", "20")); //SORAI : 30 sec
        }
    }
    ...
    public Object executeStart(MultiSession session, GeneralFrame generalFrame) throws Exception {
        ...
        logger.debug("# upgradeSequenceNumber={}", bd.getFwVersion());
        logger.debug("# fwVersion={}", bd.getOptionalVersion());
        logger.debug("# fwModel={}", bd.getOptionalModel());
        ...
        logger.debug("### STEP=[{}] Session write data => {}", UpgradeCommand.UpgradeStartRequest, newGFrame.toString());
    ...
    public Object executeTransaction(MultiSession session, GeneralFrame gFrame) throws Exception {
        ...
        try {
            ...
            case UpgradeStartResponse:
                ...
                logger.debug("### STEP=[{}], ModemId={}, MeterId={}, GeneralFrame = {}, TakeOver={},  Address={}", firmwareFrame.get_upgradeCommand(), bd.getModemId(), bd.getMeterId(), newGFrame.toString(), bd.isTakeOver(), address);
            ...
            case UpgradeEndResponse:
                logger.debug("### STEP=[{}], ModemId={}, MeterId={}, ResultCode={}", firmwareFrame.get_upgradeCommand(), bd.getModemId(), bd.getMeterId(), firmwareFrame.get_imageCode().name());
                ...
                EV_TW_200_65_0_Action action1 = new EV_TW_200_65_0_Action();
                action1.makeEvent(TargetClass.valueOf(bd.getModemType().name()), bd.getModemId(), bd.getTargetClass(), openTime, elapseTime, resultCode, resultCode.getDesc(), "HES");

                action1.updateOTAHistory((bd.getOtaTargetDeviceType() == DeviceType.Modem ? bd.getModemId() : bd.getMeterId()), bd.getOtaTargetDeviceType(), openTime, resultCode, "");
                ...
                case NoError:
                    ...
                    break;
                case CRCFail:
                    logger.debug("### STEP=[{}], ModemId={}, MeterId={}, Modem OTA Fail~~~!!", firmwareFrame.get_upgradeCommand(), bd.getModemId(), bd.getMeterId());
                    logger.debug("### STEP=[{}], ModemId={}, MeterId={}, Modem OTA Fail~~~!!", firmwareFrame.get_upgradeCommand(), bd.getModemId(), bd.getMeterId());
                    logger.debug("### STEP=[{}], ModemId={}, MeterId={}, Modem OTA Fail~~~!!", firmwareFrame.get_upgradeCommand(), bd.getModemId(), bd.getMeterId());
            ...
            case UpgradeImageInstallResponse:
                ...
                case NoError:
                    //result = true;
                    newGFrame = CommandNIProxy.setGeneralFrameOption(FrameOption_Type.Command, NIAttributeId.ModemScheduleRun);
                    ...
                    if("TW".equals(target.getNameSpace())) {
                        if (target.getReceiverType().equals("MMIU") || target.getTargetType() == McuType.MMIU) {
                            if (target.getProtocol() == Protocol.SMS || target.getProtocol() == Protocol.GPRS) { // MBB Modem / TCP
                                newGFrame.setNetworkType(NetworkType.MBB);
                            }
                        } else if (target.getReceiverType().equals("SubGiga")) {
                            if (target.getProtocol() == Protocol.IP) { // RF Modem / UDP
                                newGFrame.setNetworkType(NetworkType.Sub1Ghz_SORIA);
                            } else {
                                newGFrame.setNetworkType(NetworkType.Ethernet);
                            }
                        }
                    }
                    
                    byte[] sendData = new GeneralDataFrame().make(newGFrame, null);
                    session.write(sendData);
                    logger.debug("### STEP=[{}] Session write => [{}][{}]", FrameOption_Type.Command, newGFrame.toString(), Hex.decode(sendData));
                 ...
                 */
                EV_TW_200_66_0_Action action2 = DataUtil.getBean(EV_TW_200_66_0_Action.class);
                //String msg = "F/W Version=[" + bd.getFwVersion() + "] ElapseTime = [" + elapseTime + "]";
                String msg = "ElapseTime = [" + elapseTime + "]";
                action2.makeEvent(TargetClass.valueOf(bd.getModemType().name()), bd.getModemId(), bd.getTargetClass(), 
                        openTime, OTA_UPGRADE_RESULT_CODE.OTAERR_NOERROR, msg, "HES");
                DataUtil.getBean(FirmwareUtil.class).updateOTAHistory(
                        bd.getOtaTargetDeviceType() == DeviceType.Modem ? bd.getModemId() : bd.getMeterId(),
                                bd.getOtaTargetDeviceType(), openTime, OTA_UPGRADE_RESULT_CODE.OTAERR_NOERROR, msg, null, "OTA Result");

                logger.debug("#### [Upgrade Fininsed] OTATargetType={}, Meter={}, Modem={}, FWVersion={}, ElapseTime={} F/W Upgrade finished. result = {}!!! ", bd.getOtaTargetDeviceType(), bd.getMeterId(), bd.getModemId(), bd.getFwVersion(), elapseTime, result);
                logger.debug("#### [Upgrade Fininsed] OTATargetType={}, Meter={}, Modem={}, FWVersion={}, ElapseTime={} F/W Upgrade finished. result = {}!!! ", bd.getOtaTargetDeviceType(), bd.getMeterId(), bd.getModemId(), bd.getFwVersion(), elapseTime, result);
                logger.debug("#### [Upgrade Fininsed] OTATargetType={}, Meter={}, Modem={}, FWVersion={}, ElapseTime={} F/W Upgrade finished. result = {}!!! ", bd.getOtaTargetDeviceType(), bd.getMeterId(), bd.getModemId(), bd.getFwVersion(), elapseTime, result);

                if("TW".equals(target.getNameSpace())) { //Ghana TVWS
                    /** Notify to Observers */
                    notifyParams.put("elapseTime", elapseTime);
                    notifyParams.put("result", true);
                    notifyParams.put("resultMessage", "OTA Success - ElapseTime = " + elapseTime);
                    notifyParams.put("commandAciontFinished", false);
        ...
        if (receivedSeq == 0 || receivedSeq == sendedSeq) {
            ...
            try {
                if (!sendImage(session, bd, newGFrame)) {
                     ...
                     */
                    String openTime = DateTimeUtil.getCurrentDateTimeByFormat("yyyyMMddHHmmss");
                    EV_TW_200_63_0_Action action2 = DataUtil.getBean(EV_TW_200_63_0_Action.class);
                    action2.makeEvent(TargetClass.valueOf(bd.getModemType().name()), bd.getModemId(), bd.getTargetClass(), openTime, "HES");
                    DataUtil.getBean(FirmwareUtil.class).updateOTAHistory(
                            (bd.getOtaTargetDeviceType() == DeviceType.Modem ? bd.getModemId() : bd.getMeterId()),
                            bd.getOtaTargetDeviceType(), openTime, null, "Took OTA Command");
    ...
    protected class NeedImangeBlockTransferRetry extends TimerTask {
        ...
        public NeedImangeBlockTransferRetry(String taskName, UpgradeCommand otaStep, NI_cmdModemOTAStart_Action_TW action, MultiSession session, byte[] req, int maxRetryCount) {
    ...
    public void executeResponse(MultiSession session, GeneralFrame generalFrame) throws Exception {
        if(generalFrame.payload instanceof Command) {
            Command.Attribute.Data attributeData = ((Command)generalFrame.payload).getAttribute().getData()[0];
            if(attributeData != null) {
                byte[] attrId = attributeData.getId();
                logger.debug("[HSW] - HHH // Command Attribute : "+Hex.decode(attrId));
                BypassDevice bd = session.getBypassDevice();
                
                if(NIAttributeId.ModemScheduleRun == NIAttributeId.getItem(attrId)) {
                    long endTime = System.currentTimeMillis();
                    String elapseTime = DateTimeUtil.getElapseTimeToString((endTime - bd.getStartOTATime()));
                    
                    int status = DataUtil.getIntTo2Byte(attributeData.getValue());
                    if(status == 0) {
                        notifyParams.clear();
                        notifyParams.put("elapseTime", elapseTime);
                        notifyParams.put("result", true);
                        ...
                        new Exception("Upgrade Modem Schedule Run [" + status +"] Error");
                    }
                }
            }
        }
