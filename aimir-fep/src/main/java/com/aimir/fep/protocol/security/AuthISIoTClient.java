...
public class AuthISIoTClient {
    ...
    private ArrayList<HashMap<String, Object>> iotFunctionCallHistory = new ArrayList<HashMap<String, Object>>();
...
//			"http://127.0.0.1/oac/api/");
    ...
    private int  call_IS_IoT_Get_Random(byte[] random, Integer random_len) 
    {
    	...
    	ret = nuritelecom.IS_IoT_Get_Random(random, random_len);
    ...
    private int  call_IS_IoT_Verify_Init_Token( byte[] initToken ){
    	...
    	//
    	int ret = nuritelecom.IS_IoT_Verify_Init_Token(
...
//		this.clientInfoLen = cinfo.length;
    ...
    private int call_IS_IoT_Make_Stoken2()
    {
    	...
    	int ret = nuritelecom.IS_IoT_Make_OrgMsg_For_Stoken(
    	...
    	ret = nuritelecom.IS_IoT_SignedData_For_Stoken(
    			orgmsg, orgmsg_len
    	...
    	ret = nuritelecom.IS_IoT_Make_Stoken_With_Sigdata(
    			this.serverRandom, this.serverRandomLen //s_random, s_random_len
    			, this.serverInfo, this.serverInfoLen //sinfo_s, sinfo_len_s
    ...
    private int call_IS_IoT_Verify_Ctoken(byte[] clientToken, Integer clientTokenLen)
    {
    	...
    	retVal = nuritelecom.IS_IoT_Verify_Ctoken
...
//	    	this.SKEYLen = this.SKEY.length;
    ...
    private int call_IS_IoT_PKI_Encrypt()
    {
    	...
    	int retVal = nuritelecom.IS_IoT_PKI_Encrypt
...
//    }	
    ...
    public boolean isIotFunctionCalled(IoT_Function func)
    {
    	...
    	Iterator<HashMap<String, Object>> iterator = iotFunctionCallHistory.iterator();
...
//		}
	...
	/**
	 * @deprecated RF 모뎀의 경우 3패스 인증 후 집중기로 인증을 보내나, 파나 인증서로 변경 후에 Radius 검증을 하므로 필요없음.
	 * @param status
	 */
	...
	public void SendEvent(String message) {
		...
		try {
            DataUtil.getBean(EventUtil.class).sendEvent("Authentication Alarm",
                    TargetClass.Modem,
                    new String(this.clientInfo, "UTF-8"),
