 ...
 *
 */
public class EMnVEventLogAction extends EMnVActions {
	private static Logger logger = LoggerFactory.getLogger(EMnVEventLogAction.class);
	private EMnVCommandSubFrameType commandType;
	private RWSType rwsType;
	private ActionCommandType actionCommandType = ActionCommandType.EVENT_LOG;

	public EMnVEventLogAction(EMnVCommandSubFrameType commandType) {
	...
	public void execute(EMnVGeneralFramePayLoad gFramePayload, IoSession session, EMnVGeneralDataFrame generalDataFrame) throws Exception {
		...
		case COMMAND_RESPONSE:
			...
			case SUCCESS:
				logger.info("[{}][COMMAND_RESPONSE][{}][TRNX_ID={}][{}-{}개] Success !!!"
						, new Object[]{actionCommandType.name(), aDevice.getModemId(), aDevice.getTransactionId(), payLoad.getRwsType().getDesc(), payLoad.getEventLogList().size()});
	            if (acLogList.size() > 0) {
	                ...
	                if(rwsType == RWSType.R){
	                	for(HashMap<String, Object> map : payLoad.getEventLogList()){
		                    DataUtil.getBean(EventUtil.class).sendEvent("Equipment Notification", TargetClass.LTE, aDevice.getModemId(), map.get("TIME").toString(),
		                            new String[][] {{"message", ((EventLogCode)map.get("CODE")).getDesc()}}, new EventAlertLog());
	                	}
	                }
