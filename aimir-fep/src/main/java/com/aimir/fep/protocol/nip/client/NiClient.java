...
public class NiClient implements NotiObserver { //extends IoHandlerAdapter {
    ...
    public GeneralFrame sendCommand(Target target, GeneralFrame command, byte[] data)throws Exception {
        log.debug("Target=" + target.getTargetId() +  ", NetWorkType = " + command._networkType+", actionCommandName=" + actionCommandName);
        ...
        switch (command._networkType) {
            ...
            default:
                if (Boolean.parseBoolean(FMPProperty.getProperty("protocol.ssl.use", "false"))) {
                    ...
                    dtlsHandler = DataUtil.getBean(NiDtlsProtocolHandler.class); // new NiDtlsProtocolHandler(true, dtlsConnector, peerAddr, this.getClass().getSimpleName() + ":" + DateTimeUtil.getCurrentDateTimeByFormat(null) + ":DTLS");
                    dtlsHandler.setIsCmdAdapter(true);
                    dtlsHandler.setDtlsConnector(dtlsConnector);
                    dtlsHandler.setHandlerName(this.getClass().getSimpleName() + ":" + DateTimeUtil.getCurrentDateTimeByFormat(null) + ":DTLS");
    ...
    private void responseData(GeneralFrame command) throws Exception {
        ...
        else
        {
            ...
            log.debug("[NICL][FCACK][TID]"+tid+"[fcAck]"+command.fcAck.name());
            //byte[] ackData = null;
            Object ackData = null;
        ...
        log.debug("responseData["+responseData.toString()+"] foType["+command.foType.name()+"]");
                ...
                // INSERT END SP-575
                    switch (command.niAttributeId) {
                        ...
                        case ModemPortInformation:
                            ...
                            break;   
                        case CircuitBreaker:
                        case CircuitBreaker_GET:
                            Command modemCircuitBreakerPayload = (Command) responseData.getPayload() ;
                            byte[] modemCircuitBreakerValue = (modemCircuitBreakerPayload.getAttribute().getData()[0]).getValue();
                            command.abstractCmd = new ModemCB();
                            command.abstractCmd.decode(modemCircuitBreakerValue);
                        case BypassMeterInterface:
                        case BypassMeterInterface_GET:
                            Command modemMeterInterfacePayload = (Command) responseData.getPayload();
                            byte[] modemMeterInterfaceValue = (modemMeterInterfacePayload.getAttribute().getData()[0]).getValue();
                            command.abstractCmd = new BypassMeterInterface();
                            command.abstractCmd.decode(modemMeterInterfaceValue);
                            break;
                        default:
...
//    }
    ...
    public void observerNotify(String notiGeneratorName, Map<?, ?> params) {
        ...
        if(map.get("commandAciontFinished") == null || (boolean) map.get("commandAciontFinished")) {
