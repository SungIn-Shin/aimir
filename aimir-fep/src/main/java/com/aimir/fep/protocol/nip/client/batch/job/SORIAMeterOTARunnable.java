...
public class SORIAMeterOTARunnable implements IBatchRunnable {
	...
	public void run() {
		...
		try {
			...
			if (versionCheck) {
				...
				if (bypassResult != null && bypassResult.isSuccess()) {
					String updateVersion = newFwVersion;
					if(oldFWVersion.contains("___")) {
						String[] array = oldFWVersion.split("___");
						updateVersion = array[0] + "___" +  newFwVersion;
					}
					
					meter.setSwVersion(updateVersion);
		...
		if (!executeResult) {
			...
			action2.makeEvent(TargetClass.EnergyMeter, target.getMeterId(), TargetClass.EnergyMeter, openTime, resultCode, errMsg, "HES");
