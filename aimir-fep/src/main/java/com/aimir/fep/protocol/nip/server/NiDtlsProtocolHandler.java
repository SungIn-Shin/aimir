...
public class NiDtlsProtocolHandler extends NICommandActionHandlerAdaptor implements RawDataChannel
{
    ...
    public void receiveData(RawData rawData) {
        ...
        log.debug((isCmdAdapter == true ? "[FEP-DTLS-COMMAND-SERVER] ": "") + 
                "############## HandlerName=[" + getHandlerName() + "] NI DTLS MessageReceived, RemoteAddress=" + _peerAddr.getHostString().toUpperCase());
                    ...
                    if (node == null) {
                        ...
                        if (ns instanceof NetworkStatusSub1GhzForSORIA) {
                            ...
                            if (IPUtil.checkIPv6(ipAddr))
                                ...
                                node.setIpAddr(ipAddr);
                            node.setProtocolType(Protocol.GPRS.name());
                        ...
                        if (ns instanceof NetworkStatusSub1GhzForSORIA) {
                            ...
                            if (!node.getProtocolType().equals(Protocol.GPRS.name())) {
                                isUpdated = true;
                                node.setProtocolType(Protocol.GPRS.name());
                            }
                        }
            ...
            for (Data d : attr.getData()) {
                ...
                if (attrId == NIAttributeId.ModemInformation) {
                    ...
                    if (modem != null) {
                        ...
                        DeviceModel model = deviceModelDao.findByCondition("name", nodeKind);
