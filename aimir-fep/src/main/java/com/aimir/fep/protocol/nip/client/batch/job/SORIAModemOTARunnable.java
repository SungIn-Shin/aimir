	...
	public void run() {
		...
		try {
			...
			if (otaTargetType == OTATargetType.MODEM) {
				...
				//oldFWVersion = meter.getSwVersion(); //Soria
				//Wasion의 경우 OTA를 위한 버전 정보를 별도로 관리하기 때문에 해당 기존버전정보 + "___" + OTA를 위한 버전정보로 관리한다.
				if(meter.getSwVersion().contains("___")) {
					String[] array = meter.getSwVersion().split("___");
					oldFWVersion = array[1];
				}else {
				...
				if (model.substring(0, 3).equals("MA3")) { // Kaifa 3상인경우
				...
				} else if (model.substring(0, 3).equals("MA1")) { // Kaifa 단상인경우
					if (fwFileName.substring(0, 6).equals("KFSP_V")) {
						optionModel = fwFileName; // ex) KFSP_V05010075
					} else {
						throw new Exception("Missmatch FW information. Model=" + model + ", File=" + fwFileName);
					}
				} else if (model.substring(0, 6).equals("NRAM-3")) { //Wasion 3상인경우
					if (fwFileName.startsWith("aM300MV3.1CT")) {
						optionModel = "aM300MV3.1CT341";
					} else if (fwFileName.startsWith("aM300MV3.1DC")) {
						optionModel = "aM300MV3.1DC341";
					} else {
						throw new Exception("Missmatch FW information. Model=" + model + ", File=" + fwFileName);
					}
				} else if (model.substring(0, 6).equals("NRAM-1")) { //Wasion 3상인경우
					if (fwFileName.startsWith("aM100")) {
						optionModel = "aM100(M1EG)V1.0";
					} else {
						throw new Exception("Missmatch FW information. Model=" + model + ", File=" + fwFileName);
					}
				} else if (model.substring(0, 6).equals("NRDT-1")) { //Split Meter 경우
					throw new Exception("Not Code implement");
				}
				else {
			...
			if (versionCheck) {
				...
				action.makeEvent(modemTypeTargetClass, target.getModemId(), targetClass, openTime, "HES");
				...
				if (resultNotiParams != null && (boolean) resultNotiParams.get("result")) {
					...
					if (otaTargetType == OTATargetType.MODEM) {
						...
						if(modem != null && "TW".equals(modem.getNameSpace())) {
							if(meter.getSwVersion() != null && meter.getSwVersion().contains("___")) { //___은 Wasion에서 미터 버전을 2개로 구분할 수 있는 구분자로 사용한다.
								String[] mVers = meter.getSwVersion().split("___");
								String meterSwVer = mVers[0] + "___" +  newFwVersion;
								
								meter.setSwVersion(meterSwVer);
					...
					logger.debug("Upadate Success. {} FW Version. OldFWVersion={} => NewFWVersion={}, resultMessage={}", otaTargetType.name(), oldFWVersion, newFwVersion, resultNotiParams.get("resultMessage"));
					executeResult = true;
				}else{
		...
		if (!executeResult) {
			...
			action3.makeEvent(modemTypeTargetClass, target.getModemId(), targetClass, openTime, resultCode, errMsg, "HES");
