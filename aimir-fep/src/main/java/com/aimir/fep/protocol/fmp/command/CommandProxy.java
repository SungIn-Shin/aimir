       ...
       MBeanRegistration
{
    ...
    public Object executeByPass(Target target, Target newtarget, HashMap<String, String> params) throws Exception {
    	String cmd = params.get("cmd").toString() == null ? "" :  params.get("cmd").toString();
		if (cmd.contains("ZigBeeSTS_")) {
			cmd = cmd.replace("ZigBeeSTS_", "");
			params.put("cmd", cmd);
			
			return executeZigBeeSTSByPass(target, newtarget, params);
    ...
    /** ECG ZigBee STS용 ByPass */
	public Object executeZigBeeSTSByPass(Target target, Target newtarget, HashMap<String, String> params) throws Exception {
		...
		try {
			...
			try {
				commandAction = (CommandAction) Class.forName("com.aimir.fep.bypass.actions.CommandAction_GG").newInstance();
			} catch (Exception e) {
				throw new Exception("CommandAction Execute Error.", e);
			}
			...
			if (client.isConnected()) {
				IoSession session = bypassClient.getSession();
				session.setAttribute("meterId", params.get("meterId"));
				session.setAttribute("modemType", params.get("modemType")); 
				
				// request bypass command
				commandAction.execute(params, session, client);
				
				// receive bypass command response 
				String cmd = session.getAttribute("bypassCmd") == null ? "" : session.getAttribute("bypassCmd").toString();
				...
				if (result.equals("fail")) {
					...
					resultTable.put("reason", session.getAttribute("reason"));
				} else if (result.equals("success")) {
					
					if ("cmdSetPaymentMode".equals(cmd)) {
						resultTable.put("result", result);
					} else if ("cmdGetPaymentMode".equals(cmd)) {
						resultTable.put("result", result);
						resultTable.put("mode", session.getAttribute("mode"));
					} else if ("cmdGetRemainingCredit".equals(cmd)) {
						resultTable.put("result", result);
						resultTable.put("dateTime", session.getAttribute("dateTime"));
						resultTable.put("credit", session.getAttribute("credit"));
					} else if ("cmdSetSTSToken".equals(cmd)) {
						resultTable.put("result", result);
						resultTable.put("token", session.getAttribute("token"));
					} else if ("cmdGetSTSToken".equals(cmd)) {
						resultTable.put("result", result);
						int count = Integer.parseInt(session.getAttribute("count").toString());
						resultTable.put("count", count);
					} else if ("cmdSetSTSSetup".equals(cmd)) {
						resultTable.put("result", result);
					} else if ("cmdGetSTSSetup".equals(cmd)) {
						resultTable.put("result", result);
						resultTable.put("date", session.getAttribute("date"));
						resultTable.put("stsNumber", session.getAttribute("stsNumber"));
					} else if ("cmdSetTariff".equals(cmd)) {
						resultTable.put("result", result);
					} else if ("cmdGetTariff".equals(cmd)) {
						resultTable.put("result", result);
						resultTable.put("tariffMode", session.getAttribute("tariffMode"));
						resultTable.put("yyyymmdd", session.getAttribute("yyyymmdd"));
						resultTable.put("condLimit1", session.getAttribute("condLimit1"));
						resultTable.put("condLimit2", session.getAttribute("condLimit2"));
						
						int count = Integer.parseInt(session.getAttribute("count").toString());
						resultTable.put("count", count);
						
						for (int i = 0; i < count; i++) {
							resultTable.put("cons[" + i + "]", session.getAttribute("cons[" + i + "]"));
							resultTable.put("fixedRate[" + i + "]", session.getAttribute("fixedRate[" + i + "]"));
							resultTable.put("vatRate[" + i + "]", session.getAttribute("vatRate[" + i + "]"));
							resultTable.put("condRate1[" + i + "]", session.getAttribute("condRate1[" + i + "]"));
							resultTable.put("condRate2[" + i + "]", session.getAttribute("condRate2[" + i + "]"));
						}
						...
						resultTable.put("result", result);
						resultTable.put("yyyymmdd", session.getAttribute("yyyymmdd"));
						resultTable.put("fcMode", session.getAttribute("fcMode"));
						resultTable.put("count", session.getAttribute("count"));

						for (int i = 0; i < (int) session.getAttribute("count"); i++) {
							resultTable.put("dayType[" + i + "]", session.getAttribute("dayType[" + i + "]"));
							resultTable.put("fromHHMM[" + i + "]", session.getAttribute("fromHHMM[" + i + "]"));
							resultTable.put("endHHMM[" + i + "]", session.getAttribute("endHHMM[" + i + "]"));
						}
						...
						resultTable.put("result", result);
						resultTable.put("mode", session.getAttribute("mode"));
						resultTable.put("days", session.getAttribute("days"));
					} else if ("cmdGetPreviousMonthNetCharge".equals(cmd)) {
						resultTable.put("result", result);
						resultTable.put("m_consumption", session.getAttribute("m_consumption"));
						resultTable.put("m_cost", session.getAttribute("m_cost"));
						...
						resultTable.put("result", result);
						resultTable.put("status", session.getAttribute("status"));
						resultTable.put("moduleSerial", session.getAttribute("moduleSerial"));
						resultTable.put("moduleTypeId", session.getAttribute("moduleTypeId"));
						resultTable.put("moduleRevision", session.getAttribute("moduleRevision"));
						resultTable.put("moduleMinorVer", session.getAttribute("moduleMinorVer"));
						resultTable.put("moduleMajorVer", session.getAttribute("moduleMajorVer"));
						resultTable.put("fileCtrl", session.getAttribute("fileCtrl"));
						resultTable.put("fileTypeId", session.getAttribute("fileTypeId"));
						resultTable.put("fileRevision", session.getAttribute("fileRevision"));
						resultTable.put("fileMinorVer", session.getAttribute("fileMinorVer"));
						resultTable.put("fileMajorVer", session.getAttribute("fileMajorVer"));
						resultTable.put("fileByteSize", session.getAttribute("fileByteSize"));
					} else if ("cmdSuniFirmwareUpdateKeyWrite".equals(cmd)) {
						resultTable.put("result", result);
					} else if ("cmdSuniFirmwareUpdateKeyRead".equals(cmd)) {
						resultTable.put("result", result);
						resultTable.put("keyNumber", session.getAttribute("keyNumber"));
						resultTable.put("key", session.getAttribute("key"));
					} else if ("cmdSetRFSetup".equals(cmd)) {
						resultTable.put("result", result);
					} else if ("cmdGetRFSetup".equals(cmd)) {
						resultTable.put("result", result);
						resultTable.put("channel", session.getAttribute("channel"));
						resultTable.put("panId", session.getAttribute("panId"));
					}
				} else {
					log.error("Undefined result type [" + result + "]");
				}
			}
