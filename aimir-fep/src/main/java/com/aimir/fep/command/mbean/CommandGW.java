...
import com.aimir.fep.protocol.nip.command.BypassMeterInterface.BMInterface;
...
public class CommandGW implements CommandGWMBean {
	...
	private Object invokeBypass(Target target, Target newtarget, HashMap<String, String> params) throws Exception {
					...
					if ( smiValues.length==2 ){
						if ( Integer.parseInt(smiValue.getVariable().toString()) != ErrorCode.IF4ERR_NOERROR ) {
							...
							if (smiValue.getVariable() instanceof OPAQUE) {
								...
								DLMSWasion kaifa = (DLMSWasion) edp;
	...
	public MeterData cmdOnDemandMeter(String mcuId, String meterId, String modemId, String nOption, String fromDate,
		...
		try {
			...
			// INSERT START SP-116
			else if ("TW".equals(target.getNameSpace())) {
	...
	public void cmdBroadcast(String mcuId, String deviceId, String message) throws FMPMcuException, Exception {
		...
		final int ProcessCode = Thread.currentThread().hashCode();
			...
			try {
				...
				future.addListener(new IoFutureListener<ConnectFuture>() {
					...
					public void operationComplete(ConnectFuture future) {
						log.debug("Log Sequence [" + NuriLogSequence.getInstance().getSeq().getUnchecked(ProcessCode)+" >> "
								+ NuriLogSequence.getInstance().getSeq().getUnchecked(Thread.currentThread().hashCode()) + "] Change!");
						NuriLogSequence.getInstance().copySequence(ProcessCode, Thread.currentThread().hashCode());
					}
				});
...
//	}
	...
	public Hashtable cmdReqImagePropagate(String mcuId, int upgradeType, int control,
			String imageKey, String imageUrl, String upgradeCheckSum, 
			String imageVersion, String targetModel , int cloneCount, List<String> filterValue) throws FMPMcuException, Exception {
...
//			datas.add(DataUtil.getSMIValue(new WORD(hexKey)));
			...
			log.debug("imageVersion hex = " + Hex.decode(hexVersion));
			...
			log.debug("cmdReqImagePropagate datas ==> " + datas.toString());
...
//		log.debug("## NI command - Modem Event Log ["+mdsId+"],["+logCount+"]");
    ...
    /**
     * Get Fraud Detection log throught DLMS.
     * for Kaifa
     * 
     ...
     * @throws Exception
     */
	public Map<String,Object> cmdGetFraudDetectionLog(String modemId, String meterId, String fromDate, String toDate) throws Exception{
		Map<String,Object> result = new HashMap<String,Object>();
    	
    	log.debug("cmdGetFraudDetectionLog(" + modemId + ")");
    	
    	String obisCode = this.convertObis(OBIS.FRAUDDETECTIONLOGEVENT.getCode());
		...
		result = cmdMeterParamGet(modemId, meterId, param);
		
    	return result;
	}
	
    /**
     * Get Disconnector Control(Relay) log throught DLMS.
     * for Kaifa
     * 
     * modemId is deviceSerial
     * fromDate and toDate format : yyyymmddhhMMss
     ...
     * @throws Exception
     */
	public Map<String,Object> cmdGetDisconnectorControlLog(String modemId, String meterId, String fromDate, String toDate) throws Exception{
		Map<String,Object> result = new HashMap<String,Object>();
    	
    	log.debug("cmdGetDisconnectorControlLog(" + modemId + ")");
    	
    	String obisCode = this.convertObis(OBIS.RELAY_EVENT.getCode());
		int classId = DLMS_CLASS.PROFILE_GENERIC.getClazz();
		int attrId = DLMS_CLASS_ATTR.PROFILE_GENERIC_ATTR02.getAttr();
...
>>>>>>> .r50211
     ...
     * @throws Exception
     */
    public Map<String,Object> cmdGetStandardEventLog(String modemId, String meterId, String fromDate, String toDate) throws Exception{
    	Map<String,Object> result = new HashMap<String,Object>();
     ...
     * @throws Exception
     */
    public Map<String,Object> cmdGetPowerFailureLog(String modemId, String meterId, String fromDate, String toDate) throws Exception{
    	Map<String,Object> result = new HashMap<String,Object>();
    ...
    public Map<String,String> eventLogValueByRange(String fromDate, String toDate) throws Exception {
		...
		valueMap.put("fDayOfWeek", String.valueOf(fromCal.get(Calendar.DAY_OF_WEEK) == 1 ? "7" : (String.valueOf(fromCal.get(Calendar.DAY_OF_WEEK) - 1))));
		...
		valueMap.put("tDayOfWeek", String.valueOf(toCal.get(Calendar.DAY_OF_WEEK) == 1 ? "7" : (String.valueOf(toCal.get(Calendar.DAY_OF_WEEK) - 1))));
     ...
     * @throws Exception 
     */
    public Map<String,Object> cmdMeterParamGet(String modemId, String meterId, String param) throws Exception{
    	Map<String,Object> result = new HashMap<String,Object>();
...
//			target.setProtocol(Protocol.IP);
			...
			} else if (target.getNameSpace().equals("TW")) { // SORIA
     ...
     * @throws Exception 
     */
    public Map<String,Object> cmdMeterParamSet(String modemId, String meterId, String param) throws Exception{
    	Map<String,Object> result = new HashMap<String,Object>();
		...
		if(target != null){
			...
			if (target.getNameSpace().equals("NG")) { // IRAQ
			    ...
			    int useNiBypass = Integer.parseInt(FMPProperty.getProperty("soria.protocol.modem.nibypass.use" , "0"));
			     ...
			     if ( useNiBypass > 0){
			      bypassResult = bypassClient.excuteNiBypass("cmdMeterParamSet");
			     } else {
			      bypassResult = bypassClient.excute("cmdMeterParamSet");
			     }
     ...
     * @throws Exception 
     */
    public Map<String,Object> cmdMeterParamAct(String modemId, String meterId, String param) throws Exception{
    	Map<String,Object> result = new HashMap<String,Object>();
        ...
        if(target != null){
    		...
    		if (target.getNameSpace().equals("NG")) { // IRAQ
    		    ...
    		    /*
    		     ...
    		     if ( useNiBypass > 0){    		     
    		      bypassResult = bypassClient.excuteNiBypass("cmdMeterParamAct");
    		     }
    		     else {
    		      bypassResult = bypassClient.excute("cmdMeterParamAct");
    		     }
    			*/
...
//				target.setProtocol(Protocol.IP);
				...
				if (target.getNameSpace().equals("NG")) { // IRAQ
					...
					try  {
						...
						Object resultObj = bypassResult.getResultValue();
						if(resultObj instanceof HashMap) {
							...
							if(resultObj instanceof String)
								resultMap.put("resultValue", (String)resultObj);
							else
								resultMap.put("resultValue", "cmdGetMeterFWVersion Excute Error. please try again later.");
						}
...
//						}
	...
	private Map<String, Object> mfOTAmakeFWFile(String firmwareId, OTAExecuteType otaExecuteType) throws Exception{
		...
		String kind_model = firmware.getEquipKind();
		
		if(otaExecuteType == OTAExecuteType.EACH_BY_HES) {
			String supplier = FMPProperty.getProperty("default.supplier.name", "");
			if("ECG".equals(supplier) && "meter".equals(kind_model)) {
				//1. Firmware 파일에 붙은 '(All)' 제거
				imageKey = imageKey.substring(0, imageKey.indexOf('-'));
				//2. Wasion에서 필요한 priFix 추가
				imageKey = "WASION--" + imageKey + "-";
			}
		}
...
//				break;
	...
	public String cmdSendEventByFep(long logId, String eventStatusName) throws Exception{
	    return DataUtil.getBean(EventUtil.class).sendEvent(logId, EventStatus.valueOf(eventStatusName));
	}
...
//				edp.setMeter(meter);
				...
				if ( nOption == OnDemandOption.READ_OPTION_RELAY.getCode() ) {
					...
					result = cmdMeterParamGetWithOption(modem.getDeviceSerial(), meter.getMdsId(), param, "relaystatusall" );
					...
					result = cmdMeterParamAct(modem.getDeviceSerial(), meterId, param);
...
//				edp.setMeter(meter);
				...
				byte[] rawdata = null;
				param = obisCode+"|"+classId+"|"+attrId+"|"+accessRight+"|"+dataType+"|"+value;
				result = cmdMeterParamGet(modem.getDeviceSerial(), meterId, param);
				log.debug("cmdMeterparamGet result=[" + result.toString() + "]");
//				rawdata = (byte[])result.get("rawdata");
//				edp.parse(rawdata);
//				DLMSKaifa kaifa = (DLMSKaifa) edp;
//				result = kaifa.getRelayStatus();
	...
	public String cmdSendEvent(String eventAlertName, String  target, String activatorId, String[][] params) throws Exception{
		...
		try{
            DataUtil.getBean(EventUtil.class).sendEvent(eventAlertName,
	...
	public void cmdSendEvent2(String eventAlertName, String activatorType, String activatorId, int supplierId) throws Exception {
		...
		try {
			DataUtil.getBean(EventUtil.class).sendEvent(eventAlertName, activatorType, activatorId, supplierId);
		} catch (Exception e) {
			log.error(e, e);
		}
	}
     ...
     * @throws Exception
     */
    public Map<String,Object> cmdGetLoadProfile(String modemId, String meterId, String fromDate, String toDate) throws Exception{
    	Map<String,Object> result = new HashMap<String,Object>();
		...
		log.debug("obisCode => " + obisCode + ", classId => " + classId + ", attributeId => " + attrId);
		String param = obisCode+"|"+classId+"|"+attrId+"|null|null|"+value;
    	Map<String,Object> tmpresult = cmdMeterParamGetWithOption(modemId, meterId, param, "ondemand");
	...
	/**
     * Get Month Load Profile throught DLMS.
     * for Wasion
     * 
     * modemId is deviceSerial
     * fromDate and toDate format : yyyymmddhhMMss
     ...
     */
    @Override
	public MeterData cmdGetMonthlyBillingEventLog(String modemId, String meterId, String fromDate, String toDate)
			throws Exception {
    	Map<String,Object> result = new HashMap<String,Object>();
    	
    	log.debug("cmdGetMonthlyBillingEventLog ["+ modemId + "]["+ meterId + "][" + fromDate + "][" +toDate +"]");
    	
    	Meter meter = meterDao.get(meterId);
    	Modem modem = modemDao.get(modemId);
    	if(meter == null) {
    		throw new Exception("["+meterId+"] Does not found meter device!");
    	}
    	    	
		String obisCode = this.convertObis(OBIS.MONTHLY_BILLING.getCode());
		int classId = DLMS_CLASS.PROFILE_GENERIC.getClazz();
		int attrId = DLMS_CLASS_ATTR.PROFILE_GENERIC_ATTR02.getAttr();
		...
		log.debug("obisCode => " + obisCode + ", classId => " + classId + ", attributeId => " + attrId);
		String param = obisCode+"|"+classId+"|"+attrId+"|null|null|"+value;
    	Map<String,Object> tmpresult = cmdMeterParamGetWithOption(modemId, meter.getMdsId(), param, "ondemand");
		...
		if(mdp instanceof DLMSWasion) {
			((DLMSWasion)mdp).setModemId(modemId);
		}
		
		mdp.parse((byte[])tmpresult.get("rawData"));
		
		mdata.setMeterId(meterId);
		mdata.setParser(mdp);
		
		if(modem.getMcu() != null)			
			saveMeteringDataByQueue(modem.getMcu().getSysID(), meterId, modemId, (byte[])tmpresult.get("rawData"));
		else 
			saveMeteringDataByQueue(null, meterId, modemId, (byte[])tmpresult.get("rawData"));
		
		return mdata;
	}

	/**
     * Get Daily Load Profile throught DLMS.
     * for Wasion
     * 
     * modemId is deviceSerial
     * fromDate and toDate format : yyyymmddhhMMss
     ...
     * @throws Exception
     */
    public MeterData cmdGetDailyLoadProfile(String modemId, String meterId, String fromDate, String toDate) throws Exception{
    	Map<String,Object> result = new HashMap<String,Object>();
    	
    	log.debug("cmdGetDailyLoadProfile ["+ modemId + "]["+ meterId + "][" + fromDate + "][" +toDate +"]");
		...
		String obisCode = this.convertObis(OBIS.DAILY_LOAD_PROFILE.getCode());
		int classId = DLMS_CLASS.PROFILE_GENERIC.getClazz();
		int attrId = DLMS_CLASS_ATTR.PROFILE_GENERIC_ATTR02.getAttr();
...
//          log.info(stringBuffer.toString());
      ...
      * @throws Exception 
      */
     public Map<String,Object> cmdMeterParamGetWithOption(String modemId, String meterId, String param, String option) throws Exception{
     	Map<String,Object> result = new HashMap<String,Object>();
	    ...
	    if(target != null){
	    	log.debug("Target Info => " + target.toString());
	 		 ...
	 		 */
	 		if (target.getNameSpace().equals("NG")) { // IRAQ
	 		} else if (target.getNameSpace().equals("TW")) { // ECG TVWS
	 			Map<String, Object> params = new HashMap<String, Object>();
	 			params.put("paramGet", param);
	 			...
	 			BypassResult bypassResult = null;
				int useNiBypass = Integer.parseInt(FMPProperty.getProperty("soria.protocol.modem.nibypass.use" , "0"));
	 			if ( useNiBypass > 0 && ("ondemand".equals(option) || "sts".equals(option) || "tokenhistory".equals(option))) {
	 				bypassResult = bypassClient.excuteNiBypass("cmdMeterParamGet");
	 			}
	 			else {
	 				bypassResult = bypassClient.excute("cmdMeterParamGet");
      ...
      * @throws Exception 
      */
     public Map<String,Object> cmdMeterParamSetWithOption(String modemId, String meterId, String param, String option) throws Exception{
     	Map<String,Object> result = new HashMap<String,Object>();
         ...
         if(target != null){
     		...
     		} else if (target.getNameSpace().equals("TW")) { // TVWS
     			Map<String, Object> params = new HashMap<String, Object>();
     			params.put("paramSet", param);
     			params.put("option", option);
     ...
     public Map<String,Object> cmdMeterParamActWithOption(String modemId, String meterId, String param, String option) throws Exception{
    	 Map<String,Object> result = new HashMap<String,Object>();
     	
     	log.debug("cmdMeterParamActWithOption(" + modemId + ")");
        //Target target = CmdUtil.getNullBypassTarget(modemDao.get(modemId));
     	Target target = CmdUtil.getNullBypassTarget(meterDao.get(meterId));
...
//				stream.write(DataUtil.get2ByteToInt(offsetCount[1]));				// Poll Data - Count
				...
				stream.write(DataUtil.getByteToInt(modemPort + 1));					// Poll Data - Type
 				...
 				if(modem.getModel() != null && modem.getModel().getName() != null && !"".equals(modem.getModel().getName())
 						&& modem.getModel().getName().equals("NAMR-P20DSR")) { //for split meter
 	 				smiValue = DataUtil.getSMIValue(target.getNameSpace(), new INT(0x01));	// FILTER_METERID
 	 				datas.add(smiValue);
 	 				smiValue = DataUtil.getSMIValueByObject(target.getNameSpace(), "stringEntry", meterId); // meterId
 	 				datas.add(smiValue);
    ...
    // INSERT START SP-643
 	...
 	// INSERT START SP-681
	public Map<String, String> cmdExecDmdNiCommand(Object modemId, String requestType, String attributeId, String param ) throws Exception {
		...
		String command = "";
 		...
 		try {
 			Modem modem = null;
 			if(modemId instanceof Integer) {
 	 			modem = modemDao.get((int) modemId);
 	 			
 	 			if(modem == null)
 	 				modem = modemDao.get((String) modemId);
 			}else if(modemId instanceof String) {
 	 			modem = modemDao.get((String) modemId);
 	 			
 	 			if(modem == null)
 	 				modem = modemDao.get(Integer.parseInt((String) modemId));
 			}
...
//							jo.put("Value", Hex.decode(attrDataVal));
	...
	public Map<String, String> cmdGeneralNiCommand(String modemId, String requestType, String attributeId, String param ) throws Exception {
		...
		log.debug("cmdGeneralNiCommand END ["+attributeId+"],["+cmdData+"],["+ jo.toString() +"],[" + cmdResult +"]");
 		// return map 
		return map;
	}
 	// INSERT END SP-575
 		...
 		try {
			...
			if(modem.getModel() != null && modem.getModel().getName() != null && !"".equals(modem.getModel().getName())
				...
				for(int i = 0; i < meterIds.length; i++) {
		 			smiValue = DataUtil.getSMIValue(target.getNameSpace(), new INT(0x01));	// FILTER_METERID
		 			datas.add(smiValue);
		 			smiValue = DataUtil.getSMIValueByObject(target.getNameSpace(), "stringEntry", meterIds[i]); // meterId
		 			datas.add(smiValue);
				}
...
//							System.out.print(date + "\n");
			        ...
			        for(String modemSerial : modemIds){
			        	...
			        	meter = null;
 						...
 						if (resultmodem != null) {
 							Set meterSet = resultmodem.getMeter();
 							if (meterSet != null && meterSet.size() > 0) {
 								meter = (Meter) meterSet.toArray()[0];
 							}
 						}
...
//							log.debug("MeteringData[" + Hex.decode(metringData) + "]");						    
	...
	//INSERT START NGM-54
	@Override
	public MeterData cmdMeteringDataRequestNiCommand(String mcuId, String modemId, String meterId,
				String fromDate, String toDate) throws FMPMcuException, Exception {
			Map<String,Object> resultMap = new HashMap<String,Object>();
			Map<String,Object> result = new HashMap<String,Object>();
			...
			log.debug("cmdMeteringDataRequest(mcuId[" + mcuId + "] modem["+ modemId + "] meter[" + meterId+"] from[" +  fromDate + "] to[" + toDate + "]");
			
			modem = modemDao.get(modemId);
			if(modem == null) {
				throw new Exception("Modem[" + modemId + "] does not Exist!!");
			}
			
			meter = meterDao.get(meterId);
			if(meter == null) {
				throw new Exception("Meter[" + meterId + "] does not Exist!!");
			}
			
			modemPort = meter.getModemPort() == null ? 0 : meter.getModemPort().intValue();
	        ...
	        from.setTime(sdf.parse(fromDate.substring(0, 10)));
	        to.setTime(sdf.parse(toDate.substring(0, 10)));
	      
	    	while(from.before(to)) {
				dateList.add(sdff.format(to.getTime()));				
				to.add(Calendar.MINUTE, lpInterval * -1);
			}
	    	
	    	rowCnt = dateList.size();
	        if(rowCnt > maxCnt) { //Max Read metering Data Count
		    	sendCount = ((rowCnt % maxCnt) > 0) ?  (rowCnt / maxCnt) + 1 : (rowCnt / maxCnt);
		    }
			
	        log.debug("NI Command sendCount[" + sendCount + "] dateList[" + dateList.size() + "]");
	        if (modem.getModemType() == ModemType.MMIU && modem.getProtocolType() == Protocol.SMS) { // MBB Modem
	        	throw new Exception("cmdMeteringDataRequest is not supported MMIU.");
	        } else if (modem.getModemType() == ModemType.MMIU && (modem.getProtocolType() == Protocol.IP || modem.getProtocolType() == Protocol.GPRS)) { // Ethernet Modem
	        	Map map = new HashMap<String, String>();
				...
				try {
					for(int i=1; i<=sendCount; i++) {					
				    	...
				    	if(i == sendCount) {
				    		int num = (dateList.size() - ((sendCount - 1) * maxCnt)); //마지막 전송 날짜 파라미터 개수 계산
				    		dateArray = new String[num];
				    		for(int j=0; j<num; j++) {
				    			dateArray[j] = dateList.get(index++);
				    		}
				    		
				    		niParam.put("count", num);
				    		niParam.put("parameters", dateArray);
				    		niParam.put("meterId", meter.getMdsId());
				    	} else {
				    		dateArray = new String[maxCnt];
				    		for(int j=0; j<maxCnt; j++) {
				    			dateArray[j] = dateList.get(index++);
				    		}
				    		
				    		niParam.put("count", maxCnt);
				    		niParam.put("parameters", dateArray);
				    		niParam.put("meterId", meter.getMdsId());
				    	}
						...
						if(i != sendCount) //한번 데이터 송수신 후 무조건 세션 종료 후 통신해야 하기 때문에 일정시간 이상 딜레이
							Thread.sleep(3000);
						
						if (resultObj.getStatus() == Status.Success) {
							byte[] responseData = resultObj.getData();
							builder.append(Hex.decode(responseData));											
						} else {
					...
					if(builder.length() > 0) {
						...
						try {
							...
							if ( edp instanceof 	DLMSKaifa ) {
								((DLMSKaifa)edp).setModemId(modem.getDeviceSerial());			
								((DLMSKaifa)edp).setModemPort(modemPort);
							} else if(edp instanceof DLMSWasion) {
								((DLMSWasion)edp).setModemId(modem.getDeviceSerial());
							}
							
							edp.parse(Hex.encode(builder.toString()));
							ed.setParser(edp);
							saveMeteringDataByQueue(mcuId, meterId, modemId, Hex.encode(builder.toString()));
							txManager.commit(txStatus);
							return ed;
						}catch(Exception e) {
							log.error(e, e);
	...
	@Override
	public Map<String, Object> cmdNIBypassMeterInterface(String modemId, String meterId, String requestType, int uArt)
			throws FMPMcuException, Exception {
		log.debug("cmdNIBypassMeterInterface modemId ["+modemId+"], meterId ["+ meterId +"], requestType [" + requestType +"]");
		Map<String,Object> resultMap = new HashMap<String,Object>();
		Target target = null;
		Modem modem = null;
		Meter meter = null;
		BypassMeterInterface resultObj = null;
		HashMap niParam = new HashMap();
		...
		if(("GET").equals(requestType)) { 
			resultObj = (BypassMeterInterface)niProxy.execute(target, NIAttributeId.BypassMeterInterface_GET, niParam, null);
		} else if(("SET").equals(requestType)) {
			niParam.put("interface", uArt);
			resultObj = (BypassMeterInterface)niProxy.execute(target, NIAttributeId.BypassMeterInterface, niParam, null);
		}
		...
		if(resultObj.getmBMInterface() == null || resultObj.getmBMInterface() ==  BMInterface.NONE) {
			resultMap.put("cmdResultType", "Fail");
		} else {
			resultMap.put("cmdResultType", "Success");
			String mInterface = resultObj.getmBMStatus().getId() == 1 ? "UART1" : "UART2";
			boolean mStatus =  resultObj.getmBMStatus().getId() == 0 ? true : false;
			
			resultMap.put("Interface", mInterface);
			resultMap.put("mStatus", mStatus);
			
			log.debug("## mInterface["+mInterface+"] mStatus["+mStatus+"]");
		}
	...
	public Map<String, Object> cmdCloneOnOffViaSchedule(OTATargetType otaTargetType, OTAExecuteType otaExecuteType, String issueDate, boolean propagation, ModemCommandType modemCommandType, int cloningTime) throws Exception {
		...
		log.debug("Received params. OTAExecuteType=["+ otaExecuteType + "]" + ", issueDate=[" + issueDate + "], Propagation=[" + propagation + "], ModemCommandType=[" +modemCommandType+ "], CloningTime=["+cloningTime+"]");
		Map<String, Object> result = new HashMap<String, Object>();
		...
		try {
			...
			if (firmwareIssueHistoryList != null && 0 < firmwareIssueHistoryList.size()) {
				...
				for(FirmwareIssueHistory target : firmwareIssueHistoryList) {
					...
					EV_SP_200_64_0_Action action = new EV_SP_200_64_0_Action();
					action.makeEvent(TargetClass.SubGiga, target.getDeviceId(), TargetClass.SubGiga, openTime, otaExecuteType.name());
					action.updateOTAHistory(target.getDeviceId(), otaTargetDeviceType, openTime);
					
					try {
						switch (otaExecuteType) {
						case EACH_BY_HES:
							entryResult = executeClonOnOffByHES(target.getDeviceId(), cloneCode, cloneCount);
...
//							entryResult = temp;
							...
							break;
						case EACH_BY_DCU:
							log.debug("Target MODEM_ID = "+target.getDeviceId()+", Param=" + Hex.decode(DataUtil.append(cloneCodeData, cloneCountData)));

							entryResult = cmdExecDmdNiCommand(target.getDeviceId(), "SET", Hex.decode(NIAttributeId.CloneOnOff.getCode()), Hex.decode(DataUtil.append(cloneCodeData, cloneCountData)));
//							Map temp1 = new HashMap<String, Object>();
//							temp1.put("[EACH_BY_DCU]TestKey", "ok");
//							entryResult = temp1;

							for (java.util.Map.Entry<String, ?> e : entryResult.entrySet()) {
								log.info("[MODEM_ID:"+target.getDeviceId()+", REQUEST_TYPE:"+NIAttributeId.CloneOnOff.name()+", KEY["+e.getKey()+"], VALUE["+e.getValue()+"]");
							}
						...
						action2.makeEvent(TargetClass.SubGiga, target.getDeviceId(), TargetClass.SubGiga, openTime
								, resultCode, eventMsg, otaExecuteType.name());
						...
						openTime = DateTimeUtil.getCurrentDateTimeByFormat("yyyyMMddHHmmss");
						action3.makeEvent(TargetClass.SubGiga, target.getDeviceId(), TargetClass.SubGiga, openTime
	...
	public Map<String, String> 	executeClonOnOffByHES(String modemDeviceSerial, String cloneCode, Integer count) throws Exception {
		log.debug("## executeClonOnOffByHES [" + modemDeviceSerial + "], [" + cloneCode + "], [" + count + "]");
	...
	@Override
	public Hashtable cmdCreateTunnel2(String mcuId, String modemEuiId, int msgTimeout, int tunnelTimeout)
			throws FMPMcuException, Exception {
		// 18-02-09 ECG 적용
		log.debug("cmdCreateTunnel2("+mcuId+"," + modemEuiId +"," + msgTimeout + "," + tunnelTimeout+ ")");
		 
		Target target = CmdUtil.getTarget(mcuId);
		 		
		Vector<SMIValue> datas = new Vector<SMIValue>();
	
		// 베트남과 달리 가나는 모뎀을 Eui64Entry로 전달함.
		datas.add(DataUtil.getSMIValueByObject("eui64Entry", modemEuiId));                 
		datas.add(DataUtil.getSMIValue(new INT(msgTimeout))); 
		datas.add(DataUtil.getSMIValue(new INT(tunnelTimeout)));
	...
	@Override
	public void cmdDeleteTunnel2(String mcuId, String modemEuiId) throws FMPMcuException, Exception {
		 Target target = CmdUtil.getTarget(mcuId);
		 
		 log.debug("cmdDeleteTunnel2 ("+mcuId+"," + modemEuiId+ ")");
		 
		 Vector<SMIValue> datas = new Vector<SMIValue>();
		 
		 // modem id (EUI64, 1.14.0)
		 datas.add(DataUtil.getSMIValueByObject("eui64Entry", modemEuiId));
	...
	@Override
	public List<TunnelListInfo> cmdGetTunnelList(String mcuId, String[] modemEuiId) 
			throws FMPMcuException, Exception
	{
    
        StringBuffer buf = new StringBuffer();
        buf.append("cmdGetTunnelList MCU["+mcuId+"], ModemId[");
        for (int i = 0; i < modemEuiId.length; i++) {
            buf.append(modemEuiId[i]+",");
        }
        buf.append("]");
        log.debug(buf.toString());
        // Get MCU ID
        ...
        Vector<SMIValue> datas = new Vector<SMIValue>();
        for (int i = 0; i < modemEuiId.length; i++) {
        	datas.add(DataUtil.getSMIValueByObject("eui64Entry", ""+modemEuiId[i]));
        }
        ...
        // Distinguish return value
        SMIValue[] smiValues = null;
        List<TunnelListInfo> returnData = new ArrayList<TunnelListInfo>();
        ...
        if(obj instanceof Integer){
        	...
        	for(int i=0; i < smiValues.length; i+=10) {
                // 각 데이터는 순서대로 넘어옴.
        		String sensorId = ((OCTET)smiValues[i].getVariable()).toString();
            	int msgTimeout = Integer.parseInt(smiValues[i+1].getVariable().toString());
            	int tunnelTimeout = Integer.parseInt(smiValues[i+2].getVariable().toString());
            	int portNumber = Integer.parseInt(smiValues[i+3].getVariable().toString());
            	String createTime = ((OCTET)smiValues[i+4].getVariable()).toString();
            	String lastTimestamp = ((OCTET)smiValues[i+5].getVariable()).toString();
            	String sendPacket = ((OCTET)smiValues[i+6].getVariable()).toString();
            	String sendBytes = ((OCTET)smiValues[i+7].getVariable()).toString();
            	String recvPackets = ((OCTET)smiValues[i+8].getVariable()).toString();
            	String recvBytes = ((OCTET)smiValues[i+9].getVariable()).toString();
            	
            	TunnelListInfo data = new TunnelListInfo();
            	data.setSensorId(sensorId);
            	data.setMsgTimeout(msgTimeout);
            	data.setTunnelTimeout(tunnelTimeout);
            	data.setPortNumber(portNumber);
            	data.setCreateTime(createTime);
            	data.setLastTimestamp(lastTimestamp);
            	data.setSendPacket(sendPacket);
            	data.setSendBytes(sendBytes);
            	data.setRecvPackets(recvPackets);
            	data.setRecvBytes(recvBytes);
	...
	@Override
	public Map<String, Object> cmdSTSGeneralCommand(String modemId, String meterId, String requestType, String param)
			throws Exception {
		log.info("cmdSTSGeneralCommand::modemId["+modemId+"] meterId["+meterId+"] param["+param+"] requestType["+requestType+"]");
		Map<String, Object> resultMap = new HashMap<String, Object>();
		
		String option = "sts";
		String[] data = null;
		data = param.split("[|]");
		//param value example : obisCode+"|"+classId+"|"+attrId+"|"+accessRight+"|"+dataType+"|"+value;
		
		if(data[0] != null) {
			if(data[0].equals(DataUtil.convertObis(OBIS.TOKEN_CREDIT_HISTORY.getCode()))
					|| data[0].equals(DataUtil.convertObis(OBIS.OWE_CREDIT_THRESHOLD.getCode()))
					|| data[0].equals(DataUtil.convertObis(OBIS.FRIENDLY_CREDIT_THRESHOLD.getCode()))) {
				
				try {
					String value = null;
					if(data[0].equals(DataUtil.convertObis(OBIS.TOKEN_CREDIT_HISTORY.getCode())) && requestType.equals("GET")) {
						int num = Integer.parseInt(data[5]);
						value = data[5];
						option = "tokenhistory";
					} else if((data[0].equals(DataUtil.convertObis(OBIS.OWE_CREDIT_THRESHOLD.getCode())) 
							|| data[0].equals(DataUtil.convertObis(OBIS.FRIENDLY_CREDIT_THRESHOLD.getCode())))
							&& requestType.equals("SET")) {
						int num = Integer.parseInt(data[5]) * 100;
						value = String.valueOf(num);
					}
					
					param = data[0] + "|" + data[1] + "|" + data[2] + "|null|null|" + value; 
				}catch(Exception e) {
		...
		if(requestType.equals("GET"))
			resultMap = cmdMeterParamGetWithOption(modemId, meterId, param, option);
		else if(requestType.equals("SET"))
			resultMap = cmdMeterParamSetWithOption(modemId, meterId, param, option);
		else if(requestType.equals("ACT"))
			resultMap = cmdMeterParamActWithOption(modemId, meterId, param, option);
		else 
			throw new Exception("Can not Support Command Type. ");

		if(data[0].equals(DataUtil.convertObis(OBIS.TOKEN_CREDIT_HISTORY.getCode()))) { //get charge event log
			if(resultMap.get("rawData") != null) {
				byte[] rawData = Hex.encode(resultMap.get("rawData").toString());
				...
				try {
					txstatus = txmanager.getTransaction(null);
					Meter meter = meterDao.get(meterId);
					MeterDataParser edp = (MeterDataParser) Class.forName(meter.getModel().getDeviceConfig().getParserName()).newInstance();

					if(edp != null && edp instanceof DLMSWasion) {
						DLMSWasion mdp = (DLMSWasion)edp;
						mdp.setModemId(modemId);
						mdp.setMeter(meter);
						mdp.parse(rawData);
						
						List<LinkedHashMap<String, Object>> historyList =  mdp.getTokenChargeData();
						if(historyList != null && historyList.size() > 0) {
							String jsonData = DataUtil.getStringToListMap(historyList);
							resultMap.remove("rawData");
							resultMap.put("value", jsonData);
						} else {
							resultMap.remove("rawData");
							resultMap.put("value", "");
						}
			...
			String resultStatus = (String) resultMap.get("RESULT_VALUE");
			if(resultStatus != null && !resultStatus.isEmpty()) {
				if("Success".equals(resultStatus)) {
					String value = (String) resultMap.get("value");
					if(!"0".equals(value)) {
						double dvalue = Double.parseDouble(value) * 0.01;
						resultMap.put("value", String.format("%.2f", dvalue));
					}
				}
			}
		}
	...
	@Override
	public Map<String, Object> cmdSTSBlockTariff(String modemId, String meterId, String requestType, String param)
			throws Exception {
		//param value example : obisCode+"|"+classId+"|"+attrId+"|"+value;
		log.info("cmdSTSBlockTariff::modemId["+modemId+"] meterId["+meterId+"] param["+param+"] requestType["+requestType+"]");
		Map<String, Object> resultMap = new HashMap<String, Object>();
		...
		Target target = CmdUtil.getNullBypassTarget(meterDao.get(meterId));
		if(target != null) {
			if (target.getNameSpace().equals("TW")) {
				HashMap<String, Object> params = new HashMap<String, Object>();
				params.put("meterId", meterId);
				params.put("requestType", requestType);
				params.put("blockParam", param);
				...
				try {
					bypassClient = new BypassClient(target);     				
    	 			...
    	 			if ( useNiBypass > 0){
    	 				...
    	 				bypassResult = bypassClient.excuteNiBypass("cmdSTSMeterTariff");
    	 			} else {
    	 				log.debug("[cmdSTSBlockTariff] Using Null Bypass.");
    	 				bypassResult = bypassClient.excute("cmdSTSMeterTariff");
    	 			}
    	 			
    	 			if(bypassResult.getResultValue() instanceof Map) {
    	 				resultMap = (Map<String, Object>) bypassResult.getResultValue();
         			} else {
         				Map<String,Object> map = new HashMap<String,Object>();
         				resultMap.put("RESULT_VALUE", bypassResult.getResultValue());
         			}
				}catch (Exception e) {
					if(bypassClient != null) bypassClient.close();
				...
				resultMap.put("RESULT_VALUE", "Can not found NameSpace. please check NameSpace information.");
			}
		} else {
			resultMap.put("RESULT_VALUE", "Can not found target. please check Meter & Modem information.");
		}
	...
	public Map<String, Object> cmdSTSNetCharge(String modemId, String meterId, String param) throws Exception {
		...
		if(param == null || param.isEmpty()) {
			throw new Exception("Invalid Get Date Parameter.");
		}
		//현재 날짜의 시간을 구한다.
		if(param.length() > 6) 
		...
		Calendar paramCal = DateTimeUtil.getCalendar(param + "01");		
		long paramLong = paramCal.getTimeInMillis();
		
		SimpleDateFormat sdf = new SimpleDateFormat("yyyyMM");
		String dateTime = sdf.format(new Date()) + cal.getActualMaximum(Calendar.DAY_OF_MONTH);
		Calendar maxCal = DateTimeUtil.getCalendar(dateTime);
		long maxLong = maxCal.getTimeInMillis();
		
		Calendar minCal = DateTimeUtil.getCalendar(dateTime.substring(0,  6) + "01");
		minCal.add(Calendar.YEAR, -1);
		minCal.add(Calendar.MONTH, +1);
		long minLong = minCal.getTimeInMillis();
		
		log.debug("Beteen Max Time["+DateTimeUtil.getDateString(maxCal.getTime())+"] Min Time["+DateTimeUtil.getDateString(minCal.getTime())+"] Find Time["+DateTimeUtil.getDateString(paramCal.getTime())+"]");
		
		if(minLong > paramLong || maxLong < paramLong) {
			throw new Exception("Invalid Get Date Parameter.");	
		}
		
		int month = paramCal.get(Calendar.MONTH) + 1;
		String obisCode = null;
		
		switch(month) {
		case 1:
			obisCode = "1.0.1.219.1.101";
			break;
		case 2:
			obisCode = "1.0.1.219.1.102";
			break;
		case 3:
			obisCode = "1.0.1.219.1.103";
			break;
		case 4:
			obisCode = "1.0.1.219.1.104";
			break;
		case 5:
			obisCode = "1.0.1.219.1.105";
			break;
		case 6:
			obisCode = "1.0.1.219.1.106";
			break;
		case 7:
			obisCode = "1.0.1.219.1.107";
			break;
		case 8:
			obisCode = "1.0.1.219.1.108";
			break;
		case 9:
			obisCode = "1.0.1.219.1.109";
			break;
		case 10:
			obisCode = "1.0.1.219.1.110";
			break;
		case 11:
			obisCode = "1.0.1.219.1.111";
			break;
		case 12:
			obisCode = "1.0.1.219.1.112";
			break;
		}
		
		String params = obisCode+"|3|2|RW|||";
		
		Map<String,Object> map = cmdSTSGeneralCommand(modemId, meterId, "GET", params);
		if(map != null) {
			String resultStatus = (String) map.get("RESULT_VALUE");
			if("Success".equals(resultStatus)) {
