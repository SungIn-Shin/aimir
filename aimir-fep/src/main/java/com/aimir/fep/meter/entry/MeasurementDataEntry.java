...
//            // INSERT END SP-193 
    ...
    private Modem validateModem(MCU mcu, String ns, String ipAddr, String protocolType) throws Exception
    {
        ...
        if (modem == null) {
            ...
            // TODO 등록 이벤트
            DataUtil.getBean(EventUtil.class).sendEvent("Equipment Registration",
                    ...
                    if (getVendor() == MeterVendor.KAMSTRUP && this.vendor == 0x29) {
                        for (DeviceModel dm : vendor.getDeviceModels()) {
                            if (dm.getName().contains("OmniPower")) {
                                meter.setModel(dm);
                                log.debug("[HSW] HHH setModel:"+dm.getName());
                                break;
                            }
                        }
                    }
                    meter.setModel(vendor.getDeviceModels().get(0));
                    log.debug("[HSW] HHH setModel:"+vendor.getDeviceModels().get(0).getName());
                    
                    if (meter.getModel() != null && meter.getModel().getMdsIdPattern() != null
...
//              modemDao.update_requires_new(modem);
                ...
                // Meter Replacement
                if (orgMeter != null) {
                    DataUtil.getBean(EventUtil.class).sendEvent("Equipment Replacement",
                            TargetClass.valueOf(meter.getMeterType().getName()),
                            modem.getDeviceSerial(),
