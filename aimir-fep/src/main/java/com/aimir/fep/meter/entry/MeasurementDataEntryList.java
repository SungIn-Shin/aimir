...
public class MeasurementDataEntryList
{
    ...
    public void decode(byte[] data, String ns, String ipAddr, String protocolType)
    {
        ...
        log.debug("ns[" + ns + "] ipAddr[" + ipAddr + "] cnt=["+cnt+"] dataLen["+data.length+"] protocolType["+protocolType+"]"); // raw[" + Hex.decode(data) + "]");
        int offset = 0;
        //for(int i = 0 ; i < cnt ; i++)
        for(;;)
        {
        ...
        setEmDataCnt(len < cnt ? cnt : len);
        log.debug("### ns[" + ns + "] ipAddr[" + ipAddr + "] cnt=["+cnt+"] dataLen["+data.length+"]");
    }
