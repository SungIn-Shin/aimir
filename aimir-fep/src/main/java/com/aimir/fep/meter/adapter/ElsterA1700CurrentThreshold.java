...
/**
 * Elster A1700 Meter current A/B/C가 특정 임계치를 초과 할 경우 알람 발생
 * @author MieUn
 *
 */
public class ElsterA1700CurrentThreshold implements AdapterInterface {
    private static Log log = LogFactory.getLog(ElsterA1700CurrentThreshold.class);
    ...
    public void execute(MeterDataParser parser) {
        ...
        try{
        	inst = ((ElsterA1700)parser).getInstrument()[0];
        }catch(Exception e){
        	log.error(e, e);
        }
        ...
        if (!message.equals("")) {
            DataUtil.getBean(EventUtil.class).sendEvent("Threshold Warning",
