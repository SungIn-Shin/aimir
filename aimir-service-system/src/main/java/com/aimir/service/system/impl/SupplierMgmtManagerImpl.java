	...
	public Map<String, Object> getChargeMgmtList(Map<String, Object> condition) {
		...
		if (supplierType
			...
			for (int i = 0; i < listWM.size(); i++) {
				...
				if(listWM.get(i).get("rateRebalancingLevy") != null)
					listWM.get(i).put("RATEREBALANCINGLEVY", dfCd.format(Double.parseDouble(listWM.get(i).get("rateRebalancingLevy").toString())));
    ...
    public String updateTariffEMTable(String date, String jsonString) {
        ...
        for ( int i = 0 ; i < dataList.length() ; i++ ) {
            ...
            try {
                ...
                if (!json.isNull("HOUR") && !"".equals(json.get("HOUR"))) {
