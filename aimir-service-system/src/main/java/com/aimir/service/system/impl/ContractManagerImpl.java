...
//                  twoDepthList = new ArrayList<Map<String, Object>>();
	...
	public Map<String, Object> getContractInfo(int contractId, int supplierId) {
        ...
        Boolean isSts = result.get("isSts") == null ? false : (Boolean)result.get("isSts");
        if(isSts!=null && isSts){
        	result.put("isSts", "Enable");
        }else{
        	result.put("isSts", "Disable");
        }
        
        Boolean isNetMetering = result.get("isNetMetering") == null ? false : (Boolean)result.get("isNetMetering");
        if(isNetMetering!=null && isNetMetering){
        	result.put("isNetMetering", "Enable");
        }else{
        	result.put("isNetMetering", "Disable");
        }
...
//        if(str == null)     bool = true;
	...
	public Contract insertContract(Map<String, Object> conditionMap) {
        ...
        Boolean isSts = (Boolean)conditionMap.get("isStsMode");
        Boolean isNetMetering = (Boolean)conditionMap.get("isNetMeteringMode");
        Operator operator = operatorDao.get(operatorId);
        ...
        //STS, NetMetering 입력
        if(isSts != null){
        	contract.setIsSts(isSts);
        }
        if(isNetMetering != null){
        	contract.setIsNetMetering(isNetMetering);
        }  
	...
	public void modifyContract(Map<String, Object> conditionMap) {
        ...
        Boolean isSts = (Boolean)conditionMap.get("isStsMode");
        Boolean isNetMetering = (Boolean)conditionMap.get("isNetMeteringMode");
        String changeDate = StringUtil.nullToBlank(conditionMap.get("changeDate"));
        String changeReason = StringUtil.nullToBlank(conditionMap.get("changeReason"));
        ...
        // OPF-495 - Change Date, Change Reason 변경시
        if(beforeContract.getChangeDate() != null){
        	String beforeChangeDate = beforeContract.getChangeDate();
        	
        	if(!beforeChangeDate.equals(changeDate))
        		 addContractChangeLog(beforeChangeDate, changeDate, "changeDate", startDatetime, writeDatetime, customer, operator, contract);
        } 
        
        if(beforeContract.getChangeReason() != null){
        	String beforeChangeReason = beforeContract.getChangeReason();
        	
        	if(!beforeChangeReason.equals(changeReason))
        		addContractChangeLog(beforeChangeReason, changeReason, "changeReason", startDatetime, writeDatetime, customer, operator, contract);
        }
...
//        }
        ...
        // STS 모드 변경
        if (beforeContract.getIsSts() != null) {
            Boolean beforeSts = beforeContract.getIsSts();

            if (!beforeSts.equals(isSts)) {
                addContractChangeLog(beforeSts, isSts, "isSts", startDatetime, writeDatetime, customer, operator, contract);
            }
        } else {
            addContractChangeLog(null, isSts, "isSts", startDatetime, writeDatetime, customer, operator, contract);
        }
        
        // NetMetering 모드 변경
        if (beforeContract.getIsNetMetering() != null) {
            Boolean beforeNetMetering = beforeContract.getIsNetMetering();

            if (!beforeNetMetering.equals(isNetMetering)) {
                addContractChangeLog(beforeNetMetering, isNetMetering, "isNetMetering", startDatetime, writeDatetime, customer, operator, contract);
            }
        } else {
            addContractChangeLog(null, isNetMetering, "isNetMetering", startDatetime, writeDatetime, customer, operator, contract);
        }
...
//        contract1.setAmountPaid(contract.getAmountPaid());
        ...
        contract1.setChangeDate(contract.getChangeDate()); // OPF-495 
        contract1.setChangeReason(contract.getChangeReason()); // OPF-495 
        
        //STS, NetMetering 갱신
        if(isSts != null){
        	contract1.setIsSts(isSts);
        }
        if(isNetMetering != null){
        	contract1.setIsNetMetering(isNetMetering);
        }                        
...
//        }
    ...
    /**
     * STS 모드에 대해 선택할 옵션을 반환
     * @return Enable(1) or Disable(0)
     */
    public List<Object> getStsModeList(){
    	List<Object> returnList = new ArrayList<Object>();

		for (StsMode s : StsMode.values()) {
        	Map<String, Object> resultMap = null;
        	resultMap = new HashMap<String, Object>();
        	resultMap.put("id", s.getTof());
        	resultMap.put("code", s.getCode());
        	resultMap.put("name", s.name());
        	returnList.add(resultMap);
        }
    ...
    /**
     * NetMetering 모드에 대한 옵션을 반환
     * @return Enable(1) or Disable(0)
     */
    public List<Object> getNetMeteringModeList(){
    	List<Object> returnList = new ArrayList<Object>();

		for (NetMeteringMode s : NetMeteringMode.values()) {
        	Map<String, Object> resultMap = null;
        	resultMap = new HashMap<String, Object>();
        	resultMap.put("id", s.getTof());
        	resultMap.put("code", s.getCode());
