...
//	  			infoWin.mouseOver = null;
	...
	{
		var content ='<b>'
			+ '<a href="javascript:showMcuMap(\'mcu\',' + mcuId + ',\'' + sysId + '\',' + lat + ',' + lng + ',0)" style="text-decoration: underline">'
			+ 'DCU: ' + sysId + '</a>'
			+ '<br/>'
			+ '<em class="btn_bluegreen"><a href="javascript:getMcuInfo('
			+ mcuId + ')">Event Time</a></em>'
    ...
    function getLocationList() {
    	$.getJSON('${ctx}/gadget/device/getLocationListForMap.do', {
    ...
    function showLocationMap(locName){
        ...
        if ( locationName == "-"){
   	    ...
   	    var kmlUrl = '${ctx}/kml/data/map_' + locationName + '.kml';
	...
	function showDsoMap(deviceClass, id, locName, latitude, longitude,altitude){
		// Delete Marks and Lines
		clearMap();
		clickFromTrigger = true;
		// Road New kml file
   	    var kmlUrl = '${ctx}/kml/data/map_' + locName + '.kml';
	...
	function showSearchedList2(responseText, status) {
		...
		for (var i = 0; i < result.length; i++) {
			...
			if ( !kmlFile || kmlFile== ""){
				...
				items += cvAddress;
				items += '</span>';
				items += '<span class="sm_btn" ><a href="javascript:;" onclick=showMcuMap('
					+ '\'' + className +'\',' + 
					+ id + ',\'' 
					+ node.sysID +'\',\''
					+ node.latitude+'\',\''
        ...
        .getJSON(
                ...
                function(returnData) {
                    ...
                    if (returnData.rtnStr == 'Success') {
                        ...
                        detailContents = returnData.detail;
...
//                             }
	...
	function initialize() {
		...
		map = new google.maps.Map(mapDiv, {
				center: new google.maps.LatLng(5.6, -0.5),	// Ghana - Accra
				zoom : 7,
	...
	<div class="gadget_body">
		...
		<div class="box-bluegradation2 googlemap-search" style="height: 220px;">
			<ul>
				<li class="padding">
					<span>
						<select id="sLocation" name="select" style="width: 120px;">
                                <option value="">"Select Location"</option>
                        </select>        
                        </span>
                        <span>
						    <div id="btn">
        						<ul><li><a href="javaScript:showLocationMap('');" class="on" id="showLocationBtn">View Location Map</a></li></ul>
    						</div>
						</span>
