    ...
    function contractAdd() {
        ...
        $.getJSON('${ctx}/gadget/system/customerMax.do?param=serviceType', {supplier:supplier, customerNo:customerNo_},
            function(json) {
                ...
                // STS mode 선택 (element이름이 customerMax.jsp에 있는것과 같으면 정상적으로 생성되지 않음)           
                var stsModeList = json.stsModeList;
                $('#isStsModeU2').pureSelect(stsModeList);
                $('#isStsModeU2 option[value="false"]').attr("selected", "true");
                $('#isStsModeU2').selectbox();                
                
                // NetMetering mode 선택 
                var netMeteringList = json.netMeteringList;
                $('#isNetMeteringU2').pureSelect(netMeteringList);
                $('#isNetMeteringU2 option[value="false"]').attr("selected", "true");
                $('#isNetMeteringU2').selectbox();
        ...
        */
        ...
        $.ajax({
            ...
            success:function(json,status) {
                ...
                if ( json.creditType.code == "2.2.1" ) {  //2.2.1 선불
                    //숫자가 아닐경우.
   ...
   *********** */
    ...
    */
    ...
    function contractAddResult(responseText, status) {
		if(responseText.result == "success" &&  responseText.needInitToken == "true"){
			var initTokenArrears = responseText.initArrears;
			var targetId = responseText.targetId;
			Ext.Msg.alert('<fmt:message key='aimir.message'/>', 'Success. And Create an Initial Token..['+ Number(initTokenArrears)+']');
			//ajax (이니셜 토큰 생성)		 
				$.getJSON('${ctx}/gadget/device/command/cmdSetInitialCredit', { target:targetId, initArrears:initTokenArrears },
						function(json) {
							var resultText = json.rtnStr;
							Ext.Msg.alert('<fmt:message key='aimir.message'/>',resultText);
					});
		}else{
...
<!--                            <tr><td class="bold withinput"><fmt:message key="aimir.contract.receioptNo"/></td>
                                ...
                                <!-- 기존 customerMax.jsp에서 사용하는 아이디: prepaymentStatusTr6 -->
                                <tr id="pane-creditType-prepay5"">
                                  		<!-- STS 모드 여부  --> 
                                    	<td class="bold withinput"><fmt:message key="aimir.sts.module" /></td>
                                        <td >
                                            <span>
                                                <select id="isStsModeU2">
                                                </select>
                                            </span>
                                        </td>
	                                    <!--  NetMetering 계약 여부  -->     
