        ...
        $('#_stsInfoTab').click(function() {
        	$.post("${ctx}/gadget/prepaymentMgmt/getSelectBoxData.do",
                    function(json) {
                        ...
                        $("#stsCmd").loadSelect(stsCmdArr);
                        $("#stsCmd option:eq(0)").replaceWith("<option value=''>" + allStr + "</option>");
                        $("#stsCmd").val("");
                        $("#stsCmd").selectbox();
            ...
            window.setTimeout(function(){getSTSLog();}, 500);
        });
...
*/
                        ...
                        $("#creditTypeU").change(function(value){   
                            ...
                            if (selectedText == "2.2.1") { //prepay
                                ...
                                $('#prepaymentStatusTr6').show();

                                //버튼을 위로 끌어올릴 필요가 없어보여서 주석처리함.
                                //$('.contractUpdate').css('bottom', '50');
                                $('#meterDiv2').css('left',885);

                            } else if(selectedText == "2.2.2"){ //emergency 
                            	$("#prepaymentStatusTr").hide();
                                $("#prepaymentStatusTr2").hide();
                                $("#prepaymentStatusTr3").hide();
                                $("#prepaymentStatusTr4").hide();
                                $('#prepaymentStatusTr5').hide();
                                // sts, netmetering only
                            	$('#prepaymentStatusTr6').show();
                                
                                // prepay(2.2.1)의 화면을 따름.
                            	$('#meterDiv2').css('left',865);
                                ...
                                $("#prepaymentStatusTr4").hide();
                                $('#prepaymentStatusTr5').hide();
                                $('#prepaymentStatusTr6').hide();

                                $(".contractUpdate").css("bottom", "-5");
                                $('#meterDiv2').css('left',865);
                            }
                        });
...
//                        'receiptNumber' : $('#receiptNoU').val(),//추가
                        ...
                        'preMdsId' : preMdsId,
                        'isSts' : $('#isStsModeU').val(),
                        'isNetMetering' : $('#isNetMeteringU').val(),                        
...
//                        'receiptNo' : $('#receiptNoU').val(),//추가
                        ...
                        "prevContractId" : selectContractId,
                        "isPartpayment" : isPartpayment,
                        'isSts' : $('#isStsModeU2').val(),
                        'isNetMetering' : $('#isNetMeteringU2').val(),    
                        "oldArrears" : $("#oldArrearsU").val(),
        ...
        */
        ...
        $('#stsSetPayment').click(function() {
            stsFunction.setPayment();
        });
        
        $('#stsGetPayment').click(function() {
        	stsFunction.getPayment();
        });
        
        $('#stsSetTariff').click(function() {
        	stsFunction.setTariff();
        });
        
        $('#stsGetTariff').click(function() {
        	stsFunction.getTariff();
        });
    ...
    function setDate() {
        ...
        $('#oneMonthD').click(function() { calcDateSet(year, month, 0, 'D'); });
        $('#threeMonthD').click(function() { calcDateSet(year, month, -3, 'D'); });
        $('#sixMonthD').click(function() { calcDateSet(year, month, -6, 'D'); });
        $('#oneYearD').click(function() { calcDateSet(year, month, -12, 'D'); });
    };
...
//                    $('#amountPaidD').val(data.contract.amountPaid);
                    ...
                    if (creditTypeCode == '2.2.1') {
                        ...
                        //선불타입일때, STS와 NetMetering 출력
                        $('#stsMode').val(data.contract.isSts);
                        $('#netMeteringMode').val(data.contract.isNetMetering)
                        ...
                        //버튼 위치 조정하지 않아도 될것 같아서 주석처리.
                        //$('.contractInfoUpdateForm').css('bottom', '50');
                    } else if(creditTypeCode == '2.2.2') {  //emergency
                        ...
                        $('#prepaymentTr06').show();
                      	...
                      	//emergency에서도 STS와 NetMetering 출력
                        $('#stsMode').val(data.contract.isSts);
                        $('#netMeteringMode').val(data.contract.isNetMetering)
                        ...
                        $('#currentArrears').val("");
                        $('#chargeAvailable').val("");
                        $('.contractInfoUpdateForm').css('bottom', '-5');
                    } else {
                        $('#prepaymentTr01_1').hide();
                        $('#prepaymentTr01_2').hide();
                        $('#prepaymentTr01_3').hide();
    ...
    var stsLogStore;
    var stsLogGridOn = false;
    var stsLogGrid;
    var stsLogColModel;
    function getSTSLog() {
    	
    	var pageSize = 4;
        var width = $("#stsLogList").width();
        stsLogStore = new Ext.data.JsonStore({
            autoLoad: {params:{start: 0, limit: pageSize}},
            url: "${ctx}/gadget/prepaymentMgmt/getSTSHistory.do",
            baseParams:{
            	meterNumber: $('#meterMdsId').val(),
                cmd: $('#stsCmd').val(),
                startDate: $('#startYearD').val()+$('#startMonthD').val()+"00000000",
                endDate: $('#endYearD').val()+$('#endMonthD').val()+"31235959",
                result: $('#stsResult').val(),
                supplierId: supplierId
            },
            totalProperty: 'totalCount',
            root:'result',
        ...
        stsLogColModel = new Ext.grid.ColumnModel({
            columns: [
        ...
        if(stsLogGridOn == false) {
        	stsLogGrid = new Ext.grid.GridPanel({
                  store: stsLogStore,
                  colModel : stsLogColModel,
                  autoScroll:false,
                  width: width-20,
                  height: 145,
                  stripeRows : true,
            ...
            stsLogGrid.setWidth(width);
            var bottomToolbar = stsLogGrid.getBottomToolbar();
            stsLogGrid.reconfigure(stsLogStore, stsLogColModel);
            bottomToolbar.bindStore(stsLogStore);
        }
...
//        $("#receiptNoU").val(info.receiptNumber);
    ...
    var stsFunction = {
    	setPayment: function(meterId) {
    		var paymentMode = '';
    		
            var searchWin =  new Ext.Window({
            	title: '<b><fmt:message key='aimir.setPaymentMode'/><b/>',
                  ...
                  items:{
                    ...
                    items: [{
                      ...
                      store: new Ext.data.JsonStore({
                    	  ...
                    	  autoLoad: true,
                    	  data: {datas: [
 						  	{"code":'04',"name":'Prepaid'},
 						  	{"code":'08',"name":'Postpay'}
                    	  ]}
                      }),
					...
					{ 
						xtype: 'datefield', fieldLabel: 'Apply Date', id: 'paymentDate_id', name: 'paymentDate_name', anchor: '100%'
					}
                    ]
                  }
              	}],
              	buttons: [{
                    ...
                    handler: function() {
                  	  ...
                  	  if((flag && (Ext.getCmp('paymentMode_id').value == null || Ext.getCmp('paymentMode_id').value == '')) || (Ext.getCmp('paymentMode_id').value == 'Select...')) {
                  		  Ext.Msg.alert("","Please select Payment Mode");
                  		  flag = false;
                  		  return flag;
                  	  }
                  	  
					  if (flag && (Ext.getCmp('paymentDate_id').value == null || Ext.getCmp('paymentDate_id').value == '')) {
	          			  Ext.Msg.alert("","Please input apply date.");
	          			  flag = false;
	          			  return flag;
					  }

					  var yyyymmdd = Ext.getCmp('paymentDate_id').getValue().format('Ymd');
                  		  ...
                  		  Ext.Msg.show({
                         		...
                         		fn : function(btn) {
    			                	if(btn == 'ok') {
    			                		...
    			                		$.post("${ctx}/gadget/device/getMeterFromSerial.do",
    			                                {mdsId: $('#meterMdsId').val()},
    			                                function(json) {
    			                                	var meterId =  json.meter.id;
    			                                	Ext.Msg.wait('Waiting for response.', 'Wait !');
		...
		var optionVal = '';
		var searchWin = new Ext.Window({
		  title: '<b><fmt:message key='aimir.getPaymentMode'/><b/>',
		  modal: true, closable:true, resizable: true,
		  ...
		  items:[{
			  ...
			  items:{
				...
				frame: false, border: false,
				items: [{
				  xtype: 'label', html:'<div style="text-align:left;">' + 'Please select the mode.' +'</div>',  anchor: '100%'
				},{
				  xtype: 'combo',
				  id:'payMode_id', name: 'payMode_name', value:'Select...',          
				  ...
				  store: new Ext.data.JsonStore({
					  ...
					  autoLoad: true,
					  data: {datas: [
						 {"code":0,"name":'Active Paymode'},
						 {"code":1,"name":'Passive Paymode'}
					  ]}
				  }),
				  valueField: 'code', displayField: 'name',
				  anchor: '100%',
				  listeners: {
					select : function(combo, record, index){
					  Ext.getCmp('payMode_id').setValue(record.data.name);
		  ...
		  buttons: [{
				...
				handler: function() {
				  ...
				  if((flag && (Ext.getCmp('payMode_id').value == null || Ext.getCmp('payMode_id').value == ''))
						  || (Ext.getCmp('payMode_id').value == 'Select...')) {
            ...
            var searchWin = new Ext.Window({
              ...
              items:[{
                  ...
                  items:{
                    ...
                    items: [
    						{
    	                      xtype: 'label', html:'<div style="text-align:left;">' + 'Please input tariff Information.' + '</div>' + '<br/>',  anchor: '100%'
    	                    },
    	                    {
    	                        xtype: 'combo',
    	                        ...
    	                        store: new Ext.data.JsonStore({
    	                          autoLoad   : true,
    	                          baseParams: {serviceTypeId : tempServiceType, supplierId : supplierId},
    	                          url: '${ctx}/gadget/system/supplier/getTariffTypeByServiceId.do',
    	                    ...
    	                    {
    	                  	  	xtype: 'textfield', fieldLabel: 'Gov Levy', id: 'govLey_id', anchor: '100%'  
    	                    },
    	                    {
    	                  	  	xtype: 'textfield', fieldLabel: 'Street Light Levy', id: 'streetLightLevy_id', anchor: '100%'  
    	                    },
    	                    {
    	                  	  	xtype: 'textfield', fieldLabel: 'VAT', id: 'vat_id', anchor: '100%'  
    	                    },
    	                    { 
    							xtype: 'label', html: '<br/><b>' + '[NOTICE]' + '</b>' + '<br/>',  anchor: '100%'
    						},
    						{ 
    							xtype: 'label', html: '<font color=\'red\'><b>*</b></font> If you are not sure how to write, please refer to the tariff information in the suplier gadget.' + '<br/>',  anchor: '100%'
    						}
              ...
              buttons: [{
                ...
                handler: function() {
              	  ...
              	  if(flag && ($('#govLey_id').val() == null || $('#govLey_id').val() == '') || isNaN($('#govLey_id').val())) {
              		  Ext.Msg.alert("","Please input Goverment Levy.");
              		  flag = false;
              		  return flag;
              	  }
              	  
              	  if(flag && ($('#streetLightLevy_id').val() == null || $('#streetLightLevy_id').val() == '' || isNaN($('#streetLightLevy_id').val()))) {
              		  Ext.Msg.alert("","Please input Street Light Levy.");
              		  flag = false;
              		  return flag;
              	  } 
              	  
              	  if(flag && ($('#vat_id').val() == null || $('#vat_id').val() == '' || isNaN($('#vat_id').val()))) {
            		  Ext.Msg.alert("","Please input VAT.");
            		  flag = false;
            		  return flag;
            	  }
            	  ...
            	  var yyyymmdd = Ext.getCmp('tariffDate_id').getValue().format('Ymd');
              	  var tariffParam = {
                 	'tariffType' : tariffType,
              	  	'yyyymmdd' : yyyymmdd,
              	  	'govLey' : $('#govLey_id').val(),
              	  	'streetLightLevy' : $('#streetLightLevy_id').val(),
              	  	'vat' : $('#vat_id').val()
              	  }
              	  ...
              	  if(flag) {
              		...
              		$.getJSON('${ctx}/gadget/system/supplier/getSTSTariff.do'
  	            	  ...
  	            	  stsFunction.drawTariffGrid(tariffParam, json.tariffInfo); 
                    });		
              	  }
                }
              }, {
            ...
            var searchWin = new Ext.Window({
              ...
              items:[{
                  ...
                  items:{
                    ...
                    frame: false, border: false,
                    items: [{
                      xtype: 'label', html:'<div style="text-align:left;">' + 'Please select tariff mode.</br>' +'</div>',  anchor: '100%'
                    },{
                      xtype: 'combo',
                      ...
                      store: new Ext.data.JsonStore({
                    	  ...
                    	  autoLoad: true,
                    	  data: {datas: [
 						  	 {"code":0,"name":'Current tariff (Active)'},
 						  	{"code":1,"name":'Future tariff (Passive)'}
                    	  ]}
                      }),
                      valueField: 'code', displayField: 'name',
                      anchor: '100%',
                     		...
                     		fn : function(btn) {
			                	if(btn == 'ok') {
			                		...
			                		$.post("${ctx}/gadget/device/getMeterFromSerial.do",
			                                ...
			                                function(json) {
			                                	var meterId = json.meter.id;
			                                	Ext.Msg.wait('Waiting for response.<br/><font color="red"><b>*</b></font> It will take about 5 minutes.', 'Wait !');
			                                	$.getJSON('${ctx}/gadget/device/command/cmdGetTariffSingle.do', {
							       					'supplierId':supplierId,		
							       					'target' : meterId,
		            	                               ...
		            	                               Ext.Msg.alert('<fmt:message key='aimir.message'/>', result);
							                       });
			                                }
			                           );		
			                	}
    	...
    	addRecordData: function() {
        	...
        	if(store.data.length > 0) {
	        	...
	        	flag = stsFunction.validate(tariffSTSGrid,tariffSTSStore.data.length-1,preRecord.cons,"<fmt:message key='aimir.mandatoryValue'/>",0);
                ...
                if(flag) {
                	flag = stsFunction.validate(tariffSTSGrid,tariffSTSStore.data.length-1,preRecord.fixedRate,"<fmt:message key='aimir.mandatoryValue'/>",1);
                }
                ...
                if(flag) {
                	flag = stsFunction.validate(tariffSTSGrid,tariffSTSStore.data.length-1,preRecord.varRate,"<fmt:message key='aimir.mandatoryValue'/>",2);
                }
                ...
                if(flag) {
                	flag = stsFunction.validate(tariffSTSGrid,tariffSTSStore.data.length-1,preRecord.condRate1,"<fmt:message key='aimir.mandatoryValue'/>",3);
                }
                ...
                if(flag) {
                	flag = stsFunction.validate(tariffSTSGrid,tariffSTSStore.data.length-1,preRecord.condRate2,"<fmt:message key='aimir.mandatoryValue'/>",4);
                }
    	...
    	validate: function(grid,rec,data,msg,row) {
    		var bol;    	   
     	   if(data == null || (data+"" == "")) {
     		   bol = false;     
    	...
    	validateMandatory: function(grid,msg) {
    		$.ajaxSetup({
    	        async : false
    	    });
    	   ...
    	   for (var i = 0; i<items.length; i++) {
				...
				var flag = stsFunction.validate(grid,i,obj.cons,msg,0);
    		   
				if (flag && (isNaN(obj.cons) || (obj.cons + "").indexOf(".") >= 0)) {
	        		flag = false;
	        		Ext.Msg.alert("<fmt:message key='aimir.warning'/>", "Please input Number.(integer type)", function() { grid.startEditing(i, 0); return false;});
				}
               
				if (flag) {
					flag = stsFunction.validate(grid,i,obj.fixedRate,"<fmt:message key='aimir.mandatoryValue'/>",1);
				}
               
				if (flag && isNaN(obj.fixedRate)) {
               		flag = false;
               		Ext.Msg.alert("<fmt:message key='aimir.warning'/>", "Please input Number.(double type)", function() { grid.startEditing(i, 1); return false;});	
				}
               
				if (flag) {
					flag = stsFunction.validate(grid,i,obj.activeEnergyCharge,"<fmt:message key='aimir.mandatoryValue'/>",2);
				}
               
				if(flag && isNaN(obj.lifeLineSubsidy)) {
					flag = false;
                  	Ext.Msg.alert("<fmt:message key='aimir.warning'/>", "Please input Number.(double type)", function() { grid.startEditing(i, 2); return false;});	
				}
				
				if (flag) {
					flag = stsFunction.validate(grid,i,obj.lifeLineSubsidy,"<fmt:message key='aimir.mandatoryValue'/>",2);
				}
				...
				if (flag) {
					flag = stsFunction.validate(grid,i,obj.condRate1,"<fmt:message key='aimir.mandatoryValue'/>",3);
				}
               
				if (flag && isNaN(obj.condRate1)) {
					flag=false;
                  	Ext.Msg.alert("<fmt:message key='aimir.warning'/>", "Please input Number.(double type)", function() { grid.startEditing(i, 3); return false;});
				}
                     
				if(flag) {
					flag = stsFunction.validate(grid,i,obj.condRate2,"<fmt:message key='aimir.mandatoryValue'/>",4);
				}
               
				if(flag && isNaN(obj.condRate2)) {
                  	flag = false;
                  	Ext.Msg.alert("<fmt:message key='aimir.warning'/>", "Please input Number.(double type)", function() { grid.startEditing(i, 4); return false;});	
				}
    	...
    	drawTariffGrid: function(tariffParam, tariffInfo) {
    		tariffSTSStore = new Ext.data.JsonStore({
                // fields : [ {name:'cons'},{name:'fixedRate'},{name:'varRate'},{name:'condRate1'},{name:'condRate2'}]
    			fields : [ {name:'cons'},{name:'fixedRate'},{name:'condRate1'},{name:'condRate2'},/*{name:'govLey'},{name:'streetLightLevy'},{name:'vat'},*/{name:'activeEnergyCharge'},{name:'lifeLineSubsidy'}]
            });
	       ...
	       var colModel = new Ext.grid.ColumnModel({
	            ...
	            columns : [{
	                ...
	                header : "<b>Active Energy Charge</b>",
	                width : 80,
	                dataIndex : "activeEnergyCharge",
	                renderer: addTooltip,
	                editor: new Ext.form.TextField({
                        id : 'activeEnergyCharge'
                    })
	            }, {
	                header : "<b>Life Line Subsidy</b>",
	                width : 80,
	                dataIndex : "lifeLineSubsidy",
	                renderer: addTooltip,
	                editor: new Ext.form.TextField({
                        id : 'lifeLineSubsidy'
                    })
	            ...
	            /* {
	                ...
	                header : "<b>Street Light Levy</b>",
	                width : 80,
	                dataIndex : "streetLightLevy",
	                renderer: addTooltip,
	                editor: new Ext.form.TextField({
                        id : 'streetLightLevy'
                    })
	        ...
	        var imgWin = new Ext.Window({
	            ...
	            buttons : [{text : '<fmt:message key="aimir.save2"/>',
	            	handler : function() {	
	            		...
	            		flag = stsFunction.validateMandatory(tariffSTSGrid,"<fmt:message key='aimir.mandatoryValue'/>");
	            		...
	            		if (flag) {
		            		...
		            		for(var i = 0; i<records.length; i++) {
		            			...
		            			saveArr.push({
				                    ...
				                    /* 'govLey' : records[i].data.govLey,
				                    'streetLightLevy' : records[i].data.streetLightLevy,
				                    'vat' : records[i].data.vat, */
				                    'activeEnergyCharge' : records[i].data.activeEnergyCharge,
				                    'lifeLineSubsidy' : records[i].data.lifeLineSubsidy,
	                       		...
	                       		fn : function(btn) {
				                	if(btn == 'ok') {
				                		...
				                		$.post("${ctx}/gadget/device/getMeterFromSerial.do",
				                                ...
				                                function(json) {
				                                	var meterId = json.meter.id;
				                                	var meterModel = json.meter.model;
				                                	imgWin.hide(this); 
				                                	Ext.Msg.wait('Waiting for response.<br/><font color="red"><b>*</b></font> It will take about 5 minutes.', 'Wait !');
				                                	$.post('${ctx}/gadget/device/command/cmdSetTariffSingle.do',{
														'supplierId':supplierId,		            			
														'target':meterId,
														'tariffType':tariffParam.tariffType,
														'yyyymmdd':tariffParam.yyyymmdd,
														'govLey':tariffParam.govLey,
														'streetLightLevy':tariffParam.streetLightLevy,
														'vat':tariffParam.vat,
...
-->
            ...
            <li><a href="#stsInfoTab" id="_stsInfoTab">STS Information</a></li>
        </ul>
                        ...
                        <ul class="width">
                            ...
                            <li class="contractInfoDetail" id="contractInfoDetail">
                                <table class="searchoption wfree" border=0>
                                    <tr>
                                        <!-- 고객 번호-->
                                        <td class="bold withinput"><fmt:message key="aimir.customerid"/></td>
                                        <td class="padding-r20px2">
                                            <input type="text" id="customerNo" readonly class="border-trans bg-trans" style="width: 120px;"/>
                                        </td>

										<!-- 계약 번호-->
                                        <td class="bold withinput"><fmt:message key="aimir.contractNumber"/></td>
                                        <td class="padding-r20px2">
                                    ...
                                    <tr>
                                    	<!-- 공급 타입(전기,수도 등) -->
                                        <td class="bold withinput"><fmt:message key="aimir.supply.type" /></td>
                                        <td class="padding-r20px2">
                                            <input type="text" id="serviceTypeName" readonly class="border-trans bg-trans"/>
                                        </td>
                                        
                                        <!-- 공급 상태 -->
                                        <td class="bold withinput"><fmt:message key="aimir.supplystatus" /></td>
                                        <td class="padding-r20px2">
                                            <input type="text" id="statusName" readonly class="border-trans bg-trans" style="width: 120px;"/>
                                        </td>
            							
            							<!-- 공급 지역 -->
                                        <td class="bold withinput"><fmt:message key="aimir.location.supplier" /></td>
                                        <td>
                                    ...
                                    <tr>
                                        <!-- 지불타입 -->
                                        <td class="bold withinput"><fmt:message key="aimir.paymenttype" /></td>
                                        <td>
                                            <input type="text" id="creditTypeName" readonly class="border-trans bg-trans" style="width: 120px;"/>
                                        </td>

                                        <!-- sic id code (산업분류코드) -->
                                        <td class="bold withinput"><fmt:message key="aimir.sic" /></td>
                                        <td class="padding-r20px2">
                                        ...
                                        <!-- 계약전력 -->
                                        <td class="bold withinput"><fmt:message key="aimir.contract.demand.amount" /></td>
                                        <td class="padding-r20px2">
                                            <input type="text" id="contractDemand" readonly class="border-trans bg-trans"/>
                                        </td>
                                      
                                        <!-- operator 정보 -->
                                        <td id="operatorInfo1" class="bold withinput" style="display: none;"><fmt:message key="aimir.operator" /></td>
                                    ...
                                    <tr id="prepaymentTr01" >
                                    	<!-- 계약일자 -->
                                        <td class="bold withinput"><fmt:message key="aimir.contractDate" /></td>
                                        <td class="padding-r20px2">
                                            <input type="text" id="contractStartDate" readonly="readonly" class="border-trans bg-trans"/>
                                        </td> 
                                        <!-- 과거? 미수금 -->
                                        <td id="prepaymentTr01_1" style="display: none;" class="bold withinput"><fmt:message key="aimir.oldArrears" /></td>
										...
										<!-- 미수금 -->
                                        <td id="prepaymentTr01_3" style="display: none;" class="bold withinput"><fmt:message key="aimir.arrears" /></td>
                                        ...
                                        <!-- 미수금 납부상태-->
                                        <td id="prepaymentTr01_5" style="display: none;"  class="bold withinput"><fmt:message key='aimir.payment.contractCnt'/></td>
                                    ...
                                    <tr id="prepaymentTr02" style="display: none;">
                                        <!-- 바코드 -->
                                        <td class="bold withinput"><fmt:message key="aimir.barcode"/></td>
                                        <td>
                                        ...
                                        <!-- 부채 총량 -->
                                        <td id="debtAmountTitle" class="bold withinput"><fmt:message key='aimir.debtAmount'/></td>
                                        <td class="padding-r20px2">
                                            <span>
                                        ...
                                        <!-- 부채 관련... -->
                                        <td id="debtContractCntTitle" class="bold withinput"><fmt:message key='aimir.debtContractCnt'/></td>
                                        <td class="padding-r20px2">
                                            <span>
                                    ...
                                    <tr id="prepaymentTr03" style="display: none;">
                                    	<!-- 충전가능여부 -->
                                        <td class="bold withinput"><fmt:message key="aimir.charging" /></td>
                                        <td class="padding-r20px2">
                                            <input type="text" id="chargeAvailable" readonly="readonly" class="border-trans bg-trans"/>
                                        </td>
                                        <!-- 충전 상태(?) -->
                                        <td class="bold withinput"><fmt:message key="aimir.paymentstat" /></td>
                                        <td class="padding-r20px2">
                                            <span>
  		  							...
  		  							<tr id="prepaymentTr06" style="display: none;">
                                        <!-- STS 모드 여부  -->                                        
                                        <td class="bold withinput"><fmt:message key="aimir.sts.module" /></td>
                                        <td class="padding-r20px2">
                                            <span>
                                                <input name="stsMode" id="stsMode" type="text" style="width: 100px" readonly class="border-trans bg-trans"/>
                                            </span>
                                        </td>
                                        
                                        <!--  NetMetering 계약 여부  --> 
                                        <td class="bold withinput"><fmt:message key="aimir.netmetering.mode" /></td>
                                        <td class="padding-r20px2">
                                            <span>
                                                <input name="netMeteringMode" id="netMeteringMode" type="text" style="width: 100px" readonly class="border-trans bg-trans"/>
                                            </span>
                                        </td>
                                    </tr>
                            ...
                            <li class="padding" id="contractInfoUpdate" style="display: none">
                                ...
                                <table class="searchoption wfree"  border=0 >
                                    ...
                                    <tr>
                                        <!-- 공급타입 -->
                                        <td class="darkgraybold11pt withinput"><fmt:message key="aimir.supply.type" /></td>
                                        <td class="padding-r20px2">
                                    ...
                                    <tr>
                                        ...
                                        <td class="padding-r20px2">
                                            ...
                                            <span class="am_button margin-l10 margin-t1px">
                                                ...
                                                <a id="meterSearchButton2" class="on"><fmt:message key="aimir.button.search" /></a>
                                            </span>
                                        </td>
                                    </tr>
                                    <tr>
                                    ...
                                    <!-- 지불상태 항목에서 선불을 선택했을 경우 보여주는 div -->
                                    <tr id="prepaymentStatusTr">
                                        ...
                                        <!-- 미수금 -->
                                        <td class="bold withinput"><fmt:message key="aimir.arrears"/></td>
                                        <td>
                                            <input type='text' id='currentArrearsU' name="currentArrears" style="width: 150px"/>
                                        </td>
                                        <!-- Contract CNT -->
                                        <td class="bold withinput"><fmt:message key='aimir.payment.contractCnt'/></td>                      
                                        <td>
                                    ...
                                    <tr id="prepaymentStatusTr4">
                                    	...
                                    	<!-- 부채 총량 -->
                                    	<td id="debtAmountTitleU" class="bold withinput"><fmt:message key='aimir.debtAmount'/></td>
										<td>                                    	
                                    		<input type='text' id='debtAmountU' name='debtAmount' style='width: 150px' readonly/>
                                    	</td>
                                    	<!-- 부채 계약 개수(?) -->
                                    	<td id = "debtContractCntTitleU"class="bold withinput"><fmt:message key='aimir.debtContractCnt'/></td>
                                    	<td>
                                   ...
                                   <tr id="prepaymentStatusTr6">
                                  		<!-- STS 모드 여부  --> 
                                    	<td class="bold withinput"><fmt:message key="aimir.sts.module" /></td>
                                        <td >
                                            <span>
                                                <select id="isStsModeU">
                                                </select>
                                            </span>
                                        </td>
	                                    <!--  NetMetering 계약 여부  -->     
                                ...
                                <div id="btn" class="contractUpdate" style="bottom: 0px;left: 680px;">
                                    <ul>
                                        <li class="input">
    ...
    <!--    
            ...
            더이상 사용하지 않는 기능으로 주석처리함. 향후 필요 없을시 삭제 필요함.
                   ...
                   <div id="paymentLogTab" class="blueline bg-blue" style="height: 250px;">
                    ...
                    <!-- 월별 사용량 tab End -->
					<!-- STS 정보 tab start -->
                    <div id="stsInfoTab" class="blueline bg-blue" style="height: 290px;">
                        ...
                        <ul class="width">
                            <li class="padding">
                                <div id="period">
                                    <ul class="align">
                                        <li>
                                            <button id="oneMonthD" type="button" class="sm">
                                                <fmt:message key="aimir.thismonth" />
                                            </button>
                                            <button id="threeMonthD" type="button" class="sm">
                                                <fmt:message key="aimir.threemonths" />
                                            </button>
                                            <button id="sixMonthD" type="button" class="sm">
                                                <fmt:message key="aimir.sixmonths" />
                                            </button>
                                            <button id="oneYearD" type="button" class="sm">
                                                <fmt:message key="aimir.oneyear" />
                                            </button>
                                    ...
                                    <ul class="align">
                                        <li>
                                            <input type="text" id="startYearD" size="4" />
                                            <span class="withinput">
                                                <fmt:message key="aimir.year" />
                                            </span>
                                            <input type="text" id="startMonthD" size="2" />
                                            <span class="withinput">
                                                <fmt:message key="aimir.month" />
                                                ~
                                            </span>
                                            <input type="text" id="endYearD" size="4" />
                                            <span class="withinput">
                                                <fmt:message key="aimir.year" />
                                            </span>
                                            <input type="text" id="endMonthD" size="2" />
                                            <span class="withinput">
                                                <fmt:message key="aimir.month" />
                                            </span>
                                            <span class="withinput">
                                            ...
                                            <span class="withinput">
                                            	<select id="stsCmd" style="width: 200px"></select>
                                            </span>
                                            <span class="withinput">
                                                Result
                                            </span>
                                            <span class="withinput">
                                            	<select id="stsResult" style="width: 80px"></select>
                                            </span>
                                        </li>
                                        <li>
                                            <div id="btn" style="margin-right: 100px">
                                                <ul><li>
                                                        <a id="searchD" class="on">
                                                            <fmt:message key="aimir.button.search" />
                                                        </a>
                                                </li></ul>
                                            </div>
                                            <div id="btn">
                                                <ul><li>
                                                        <a id="stsSetPayment" class="on">
                                                            <fmt:message key='aimir.setPaymentMode'/>
                                                        </a>
                                                </li></ul>
                                            </div>
                                            <div id="btn">
                                                <ul><li>
                                                        <a id="stsGetPayment" class="on">
                                                            <fmt:message key='aimir.getPaymentMode'/>
                                                        </a>
                                                </li></ul>
                                            </div>
                                            <div id="btn">
                                                <ul><li>
                                                        <a id="stsSetTariff" class="on">
                                                            <fmt:message key='aimir.setTariff'/>
                                                        </a>
                                                </li></ul>
                                            </div>
                                            <div id="btn">
                                                <ul><li>
                                                        <a id="stsGetTariff" class="on">
                                                            <fmt:message key='aimir.getTariff'/>
                                                        </a>
                                                </li></ul>
                                            </div>
                                        </li>
                                ...
                                <div id="stsLogList" class="margin-t15px"> 
                                    <!-- <div id="stsLogList"></div> -->
                                </div>
                            </li>
                        </ul>
                    </div>
...
function bottomContractStatusTab(){
    ...
    $.getJSON('${ctx}/gadget/system/customerMax.do?param=contractInfoUpdateForm', 
    		{contractId: contractId , serviceType: tempServiceType, customerNo:$("#customerNo").val()},
        function(json)
        {
           ...
           // if (json.location != null) {
            ...
            // STS mode 선택            
            var stsModeList = json.stsModeList;
            $('#isStsModeU').pureSelect(stsModeList);
            if(json.isSts != null){
            	$("#isStsModeU option[value=" + json.isSts + "]").attr("selected", "true");
            }
            
            // NetMetering mode 선택
            var netMeteringList = json.netMeteringList;
            $('#isNetMeteringU').pureSelect(netMeteringList);
            if(json.isNetMetering != null){
            	$("#isNetMeteringU option[value=" + json.isNetMetering + "]").attr("selected", "true");
            }
            ...
            $("#creditTypeU").selectbox();
            $("#creditStatusU").selectbox();
            $('#isStsModeU').selectbox();
            $('#isNetMeteringU').selectbox();
