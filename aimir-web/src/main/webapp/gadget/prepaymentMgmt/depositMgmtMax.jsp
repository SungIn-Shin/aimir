	            ...
	            <span class='inline-block'> 
	              <select id='report-type' name='reportType'>
					...
					<div>
						...
						<span class="am_button margin-l10 margin-t1px" style="margin-right: 0px;">
							<a><fmt:message key="aimir.button.search"/></a>
						</span>
					</div>
				</form>
            ...
            <div id="contentWrapper">
                ...
                <form id="depositHistory">
                    <div>
                    	<span class='inline-block'>
	                        <select id='sub-type' name='subType'>
	                        	<option value='all' selected><fmt:message key='aimir.all'/>(<fmt:message key='aimir.deposit'/>)</option>
	                    		<option value='unCancelled'><fmt:message key='aimir.deposit'/></option>
	                    		<option value='cancelled'><fmt:message key='aimir.deposit'/>(<fmt:message key='aimir.cancelled'/>)</option>
								
	                    		<option value='salesUnCancelled'><fmt:message key='aimir.sales'/></option>
	                    		<option value='salesCancelled'><fmt:message key='aimir.sales'/>(<fmt:message key='aimir.cancelled'/>)</option>
	                		</select>
                		</span>
                        <span>
                    ...
                    <div>
						...
						<span id='depositSalesHistoryExcel' class="am_button margin-l10 margin-t1px">
							<a><fmt:message key="aimir.button.excel"/></a>
						</span>
						<span id='depositSalesHistoryTotalExcel' class="am_button margin-l10 margin-t1px">
							<a><fmt:message key="aimir.total"/> <fmt:message key="aimir.button.excel"/></a>
						</span>
                    </div>        
                </form>
...
var vendorHistoryParams = $.extend(true, {vendor: $("#depositHistory select[name=depositVendorList]").val(), 
    ...
    reportType:"deposit"},    
    storeParams, {limit:VENDOR_HISTORY_PAGE_SIZE});
...
var eventHandler = {
  ...
  receiptPopUp: function(rec) {
    ...
    var customerUrl = "";
    if(supplierName == "ECG") {
    	customerUrl = "${ctx}/gadget/prepaymentMgmt/prepaymentChargeReceiptPopupWithDebt.do";
    } else {
    	customerUrl = "${ctx}/gadget/prepaymentMgmt/prepaymentChargeReceiptPopup.do";
    }
    ...
    params = {
      ...
      prepaymentLogId: rec.prepaymentLogId || 0,    // OPF-441
...
*/
  ...
  tagExcelButton: function() {
      var selectedVal = $("#depositHistory option:selected").val() ;
      
   	  if (selectedVal == "all") {
    	  $("#depositSalesHistoryExcel").hide();
          $("#depositSalesHistoryTotalExcel").hide();
          $("#depositHistoryExcel").hide();
          $("#depositHistoryTotalExcel").hide();
      } else if (selectedVal == "unCancelled" || selectedVal == "cancelled") {
    	  $("#depositSalesHistoryExcel").hide();
          $("#depositSalesHistoryTotalExcel").hide();
    	  
          $("#depositHistoryExcel").show();
          $("#depositHistoryTotalExcel").show();
      } else if (selectedVal == "salesUnCancelled" || selectedVal == "salesCancelled") {
    	  $("#depositHistoryExcel").hide();
          $("#depositHistoryTotalExcel").hide();
    	  
    	  $("#depositSalesHistoryExcel").show();
          $("#depositSalesHistoryTotalExcel").show();
      }
  },
  ...
  depositHistoryTotalExcel: function() {
   ...
   depositSalesHistoryExcel: function() {
       onlyTotal = false;
       vendor = $("#depositHistory select[name=depositVendorList]").val();
       var opt = "width=600px, height=400px, left=100px, top=100px  resizable=no, status=no";
       window.open('${ctx}/gadget/prepaymentMgmt/depositExcelDownloadPopup.do', "popupWindow", opt);
       },

   depositSalesHistoryTotalExcel: function() {
       onlyTotal = true;
       vendor = $("#depositHistory select[name=depositVendorList]").val();
       var opt = "width=600px, height=400px, left=100px, top=100px resizable=no, status=no";
       window.open('${ctx}/gadget/prepaymentMgmt/depositExcelDownloadPopup.do', "totalPopupWindow", opt);
       }
};
...
var bind = function() {
    ...
    $('#depositHistory span#depositSalesHistoryExcel').click(eventHandler.depositSalesHistoryExcel);
    $('#depositHistory span#depositSalesHistoryTotalExcel').click(eventHandler.depositSalesHistoryTotalExcel);
	
    $('form.reportSearch span#monthlyReportSearch').click(eventHandler.monthlyReportSearch);
	$("form.deposit_history select[name=reportType]").bind('change', eventHandler.tagExcelButton);
};
