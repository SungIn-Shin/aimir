    ...
    <script type="text/javascript" charset="utf-8">/*<![CDATA[*/
        ...
        function searchDivInit() {
           ...
           //일자별,기간별 날짜입력창 변경시
            ...
            $(function() { $('#hourlyStartHourComboSTS')  .bind('change', function(event) { setSearchDateSTS(); } ); });
            $(function() { $('#hourlyStartMinuteComboSTS').bind('change', function(event) { setSearchDateSTS(); } ); });
            $(function() { $('#hourlyEndHourComboSTS')    .bind('change', function(event) { setSearchDateSTS(); } ); });
            $(function() { $('#hourlyEndMinuteComboSTS')  .bind('change', function(event) { setSearchDateSTS(); } ); });
            ...
            $.getJSON("${ctx}/common/getYear.do"
                ...
                ,function(json) {
                     ...
                     $('#hourlyStartHourComboSTS').numericOptions({from:0,to:23,selectedIndex:0,labels:hours});
                     $('#hourlyStartMinuteComboSTS').numericOptions({from:0,to:59,selectedIndex:0,labels:minute});
                     
                     $('#hourlyEndHourComboSTS').numericOptions({from:0,to:23,selectedIndex:23,labels:hours});
                     $('#hourlyEndMinuteComboSTS').numericOptions({from:0,to:59,selectedIndex:59,labels:minute});
                     
                     $('#hourlyStartHourComboSTS').selectbox();
                     $('#hourlyStartMinuteComboSTS').selectbox();
                     
                     $('#hourlyEndHourComboSTS').selectbox();
                     $('#hourlyEndMinuteComboSTS').selectbox();
        ...
        function validateSTSSearchCondition(){
        ...
        function getSTSSearchStartDate(){
            return $('#searchStartDateSTS').val() + $('#searchStartHourSTS').val() + "00"; //startDate            
        }

        function getSTSSearchEndDate(){
            return $('#searchEndDateSTS').val() + $('#searchEndHourSTS').val() + "59";     //endDate            
        }
                      ...
                      sm: new Ext.grid.RowSelectionModel({
                          ...
                          listeners: {
                              rowselect: function(sm, row, rec) {
                                  ...
                                  /* if(selectedModelName != null && selectedModelName == "OmniPower STS") {
                                	  $("div a[id=sts]").show();
                                	  $("#stsLog").show();
                                  } else {
                                	  $("div a[id=sts]").hide();
        ...
        var initChargeHistoryData = function() {
            ...
            chargeHistoryStore = new Ext.data.JsonStore({
                ...
                fields: ["lastTokenDate", "lastTokenDateView", "balance", "balanceView", "chargedCredit", "chargedCreditView", "currentCredit", "usedCost", "usedConsumption",
                         "consumption", "consumptionView", "keyNum", "payment", "authCode", "municipalityCode", "lastTokenId","activeImport","activeExport", "cancelCredit", "cancelReason"],
                listeners: {
                    beforeload: function(store, options){
                    options.params || (options.params = {});
                    Ext.apply(options.params, {
            ...
            chargeHistoryColModel = new Ext.grid.ColumnModel({
                columns: [
                    ...
                    ,{header: "<fmt:message key='aimir.prepayment.cancelamount'/>", dataIndex: 'cancelCredit', width: (width > mxwidth) ? width*(120/mxwidth) : 120, align:'right'}
                    ...
                    ,{
                    	...
                    	renderer: function(value, metaData, rec, index) {
 	                    	var data = rec.data;
 	                        var btnHtml = "<span style='padding:5px;''>"+data.cancelReason +"</span>"+"<span class='description_modi'><a href='#;' onclick='editDescrBtn();' ></a></span>" ;
...
//                }
 ...
 //           }
        ...
        var initSTSHistoryData = function() {
            var width = $("#stsHistoryTitle").width();
            var mxwidth = 1210;
        ...
        function getSTSHistoryData() {
        	if(validateSTSSearchCondition()){
                ...
                stsHistoryStore.load({params:{start: 0, limit: 5}});                
            }
        }
       ...
       function validateMandatory(grid,msg) {
        ...
        var cmdSTSHandler = {
	      	               		...
	      	               		fn : function(btn) {
	      		                	if(btn == 'ok') {
	      		                        ...
	      		                        $.getJSON('${ctx}/gadget/device/command/cmdSetMessage.do', {
	      		                            ...
	      		                            Ext.Msg.hide();
	      		                            var rtnStr = returnData.rtnStr;
	      		                            var rtnMsg = returnData.rtnMsg;
	      		                            if(rtnMsg == null || rtnMsg == '') {
	      		                            	$('#commandResult').val(rtnStr);
	      		                            } else {
	      		                            	$('#commandResult').val(rtnMsg);
	      		                            }
	      		                            Ext.Msg.alert('<fmt:message key='aimir.message'/>', "<fmt:message key='aimir.sts.checkhistory'/>");
	      		                        });
	      		                	} else {
	      		                		return;
	      		                	}
        	...
        	setEmergencyCredit: function() {
        		$('#commandResult').val("");        		
        		Ext.Msg.show({
               		title: '<b>Set EmergencyCredit<b/>',
               		...
               		fn : function(btn) {
	                	if(btn == 'ok') {
	                		Ext.Msg.wait('Waiting for response.', 'Wait !');
	        				var ecMode = "";
	        				if($(":input:radio[name='autoChange']:checked").val() == "false"
	        						|| $(":input:radio[name='autoChange']:checked").val() == false) {
	        					ecMode=0;
	        				} else {
	        					ecMode=1;
	        				}
        	...
        	getSpecifiedNetCharge: function() {
        		$('#commandResult').val("");        		
        		Ext.Msg.show({
        	...
        	setPaymentMode: function() {
       		...
       		cmdTcpRetry: function(){
   				...
   				$.getJSON('${ctx}/gadget/device/command/cmdTCPRetry.do', {
                       ...
                       var rtnStr = returnData.rtnStr;
                       $('#commandResult').val(rtnStr);
                       Ext.Msg.alert('<fmt:message key='aimir.message'/>', rtnStr);
                   });
        ...
        function getAsyncHistoryGrid(){
            ...
            var gridWidth = $("#asyncHistory").width();
            var pageSize = 5;
      ...
      //report window(Excel)
        ...
        function openExcelReport(exType) {
            ...
            if(exType == 'main_list'){
                ...
                header[4] = '<fmt:message key="aimir.prepayment.cancelamount"/>';
                header[5] = '<fmt:message key="aimir.hems.prepayment.balanceaftercharged"/>(<fmt:message key="aimir.price.unit"/>)'; //Total Balance(Dollar(s))
                header[6] = '<fmt:message key="aimir.hems.prepayment.transactionNum"/>'; //Transaction Number
                header[7] = '<fmt:message key="aimir.prepayment.authCode"/>'; //Authorization Code
                header[8] = '<fmt:message key="aimir.prepayment.municipalityCode"/>'; //Municipality code
                header[9] = '<fmt:message key="aimir.prepayment.balancehistory"/>'; // 파일명 : Balance History
                header[10] = '<fmt:message key="aimir.meter.value"/>'; // Meter Value
                header[11] = '<fmt:message key="aimir.button.import"/>'; // Active Import
                header[12] = '<fmt:message key="aimir.button.export"/>'; // Active Export
				header[13] = '<fmt:message key="aimir.description"/>';
  ...
  <div id="menu">
    ...
    <div id="defultTab">
	    ...
	    <div id="" class="overflow_hidden">
	        ...
	        <div class="title_basic margin_0">
	        	<span class="icon_title_blue"></span><fmt:message key="aimir.hems.prepayment.emergencycredit"/><!-- Emergency Credit -->
	        </div>
	...
	--%>
	           <!-- <div class="expect_day margin_t10"> -->
	            ...
	            -->
	                ...
	                <div id="emergencyCreditBtn" class="basic_rightbtn margin_t5">
                    	...
                    	<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="sendEmergencyModeSTS();"><span>Send STSSTS 전송</span></a> -->
	                </div>
	                <div id="emergencyCreditSTSBtn" class="basic_rightbtn margin_t5" style="display: display">
                    	<a href="#" id="sts" class="btn_blue" onClick="cmdSTSHandler.setEmergencyCredit();"><span><fmt:message key='aimir.setEmergency'/><!-- STS 전송 --></span></a>
                    	<a href="#" id="sts" class="btn_blue" onClick="cmdSTSHandler.getEmergencyCredit();"><span><fmt:message key='aimir.getEmergency'/><!-- STS 전송 --></span></a>
	                </div>
	                </div>
	            </div>
	            <!--// Emergency Credit  -->
	    ...
	    <div id="command" class="margin_t5">
			...
			<div id="relayControlButton">
	        	...
	        	<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.getRemainingCredit();"><span>Remaining Credit</span></a> -->
	        	<a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.getPreviousMonthNetCharge();"><span><fmt:message key='aimir.getPrevious'/></span></a>
	       		<a href="#" id="sts" class="btn_blue" style="display: none" onClick="showNetCharge();"><span>Get Specified<!-- For STS --></span></a>
	       		<div id="selectDate" class="floatleft tootipbox" style="display: none;">
	       			...
	       			<span>
						<div id="btn">
							<a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.getSpecifiedNetCharge();"><span><fmt:message key="aimir.button.confirm" /></span></a>
				...
				<!-- 17.09.24 Initial Credit은 잠시 삭제, 기능 보완 필요 -->
				<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.setInitialCreidt();"><span><fmt:message key='aimir.setInitialCredit'/></span></a> -->
	       		<div id="creditDiv" class="floatleft tootipbox" style="display: none;">
	       			<span style="margin: 0px; padding: 0px; color: #0f7cd0" >
	       				Token : 
	       			</span>
	       			<span style="margin: 0px; padding: 0px;">
	       				<input type="text" id="credit" name="credit" size="10px"></input>
	       			</span>
	       			<span>
						<div id="btn">
							<a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.setSTSToken();"><span><fmt:message key="aimir.button.confirm" /></span></a>
		       				<a href="#" id="sts" class="btn_blue" style="display: none" onClick="clearAndHideSetSTSTokenButton();"><span><fmt:message key="aimir.cancel" /></span></a>
						</div>	       			
	       			</span>
				</div>
				<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.setTariff();"><span>Set Tariff</span></a> -->
				<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.getTariff();"><span>Get Tariff</span></a> -->
	       		...
	       		<a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.getFriendlyCreditSchedule();"><span><fmt:message key='aimir.getFriendly'/><!-- For STS --></span></a>
				...
				<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.getPaymentMode();"><span>Get PaymentMode</span></a> -->
				<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.setPaymentMode();"><span>Set PaymentMode</span></a> -->
	       		...
	       		<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.setSTSSetup();"><span>Set STSSetup</span></a> -->
				<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.getSTSSetup();"><span>Get STSSetup</span></a> -->
	       		<!-- <a href="#" id="stsOTA" class="btn_blue" style="display: none"><span>Write F/W</span></a> -->
	       		<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.readFirmware();"><span>Read F/W</span></a> -->
	       		<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.getFwUpdateInfo();"><span>Get FwUpdateInfo</span></a> -->
	       		<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.readFwUpdateKey();"><span>Read FwUpdateKey</span></a> -->
	       		<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="cmdSTSHandler.getCIUCommStateHistory();"><span>Get CIUComm State History</span></a> -->
	       		<!-- <a href="#" id="sts" class="btn_blue" style="display: none" onClick="showFwUpdateKey();"><span>Write FwUpdateKey</span></a> -->
...
-->                    
	    ...
	    <div id="stsLog">
	        ...
	        <div class="saving_goal" style="width: 80%">
	             <table>
	                <tr>
	                    <td>
	                        <div id="hourly">
	                            <ul>
	                                ...
	                                <li class="date-space"></li>
	                                <li><select id="hourlyStartHourComboSTS" class="sm"></select></li>
	                                <li><select id="hourlyStartMinuteComboSTS" class="sm"></select></li>
	                                <li><input value="~" class="between" type="text"></li>
	                                <li><input id="hourlyEndDateSTS" class="day" type="text" readonly="readonly"></li>
	                                <li class="date-space"></li>
	                                <li><select id="hourlyEndHourComboSTS" class="sm"></select></li>
	                                <li><select id="hourlyEndMinuteComboSTS" class="sm"></select></li>
	                            </ul>
	                        </div>          
	                    </td>
    ...
    <div id="infoTab">
      ...
      <form id="infoDetailForm" class="searchoption-container margin-t10px margin-b5px">
        <div class="wrapper">
          <label class="check"><fmt:message key='aimir.prepayment.chargehistory'/></label>
        </div>
        <div class="wrapper">
