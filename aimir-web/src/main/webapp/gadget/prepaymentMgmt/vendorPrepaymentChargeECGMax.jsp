...
<body>
    ...
    <div id="chargeTab" >
      ...
      <form id="contractForm" class="searchoption-container">
        <div>
          ...
          <span id="remainingCredit" class="am_button" style="float: right; margin-right: 0px; display: none">
            <a class="on">Get Remaining Credit</a>
          </span>
          <span id="cmdTcpRetry" class="am_button" style="float: right; margin-right: 10px; display: none">
          	<a class="on">TCP Retry</a>
          </span>
          <!-- <span id="setSTSSetup" class="am_button" style="float: right; margin-right: 8px; display: none">
          	<a class="on">Set STSSetup</a>
          </span> -->
          
          <span id="setSTSToken" class="am_button" style="float: right; margin-right: 8px; display: none">
  ...
  <script type="text/javascript" charset="utf-8">/*<![CDATA[*/
    ...
    var receiptBtnArea = function(value, meta, rec) {
      ...
      if (rec.json.isCanceled && rec.json.token == null) {
          ...
          if( $("#"+id).length > 0 && ($div.children().length < 1)) {
            new Ext.Button({
              ...
              handler: function(b, e) {
                ...
                eventHandler.inputPassword2(rec);
              }
            }).render(id);
          } else {
            button.defer(100);
    ...
    var cancelBtnArea = function(value, meta, rec) {
      ...
      if ( rec.json.isCanceled != true ) {
        button.defer(100);
        return $div[0].outerHTML;
      } else if(rec.json.cancelToken != null) {
    	  return "<fmt:message key='aimir.canceled'/>"
      }
    };    
                  ...
                  "${ctx}/gadget/prepaymentMgmt/vendorSavePrepaymentChargeECG.do",
                      ...
                      function(json) {
                        ...
                        if (json != null && json.result == "success"
                        	|| json != null && json.result == "Success. But fail to transfer the token to the meter."	
                        	|| json != null && json.result == "Success. Please take your token number.") {
                  ...
                  if(reTypeValue != params.amount) {
                    ...
                    if(payType == '' || payType == 'Select...'){
                      Ext.Msg.alert("<fmt:message key='aimir.alert'/>", "<fmt:message key='aimir.msg.check.input.value'/>");
                    		...
                    		var checkInfoWin = new Ext.Window({
                                ...
                                buttons: [{
                                  ...
                                  handler: function() {
                                	  ...
                                	  if(bankCode =='' || isNaN(bankCode) || (bankCode.length > 9)) {
                    	...
                    	} else if (payTypeValue == '18.4') {
                    		Ext.Msg.alert("<fmt:message key='aimir.alert'/>", "\"Mobile\" is not available as a charging type.");
                    		return;
                    	} else {
...
//              historyListModel.setHidden(10, false);
      ...
      inputPassword: function(rec) {
    	  ...
    	  if(Ext.getCmp('CashierChangePwdWinId') == undefined) {
			  ...
			  passChangeWin = new Ext.Window({
				...
				passChangeWin.show(this);z
    	  } else {
    		  passChangeWin.show(this);
    	  }
      },
      
      inputPassword2: function(rec) {
		var changeCashierId = rec.json.casherId;
		var changeVendorId = rec.json.vendor.loginId;
		...
		Ext.Msg.show({
		    ...
		    msg: '<b><fmt:message key="aimir.hems.alert.inputPassword"/></b><br/><br/><fmt:message key='aimir.newpassword'/>:&nbsp;&nbsp;&nbsp;&nbsp;<input type="text" style="border: #888888 0.5px solid" id="textInput1"/><br/><br/><fmt:message key='aimir.userreg.confirmpassword'/>: <input type="text" style="border: #888888 0.5px solid" id="textInput2" /><br/>',
		    fn: function (button) {
		        var fp_password = document.getElementById('textInput1').value;
		        var fp_confirmPwd = document.getElementById('textInput2').value;
		        
		        if (button == 'ok') {
		        	if(fp_password == fp_confirmPwd) {
		        		Ext.Msg.show({
		        			title: changeCashierId,
		        			width: 300,
		        			buttons: Ext.Msg.OKCANCEL,
		        		...
		        		fp_password = null;
						fp_confirmPwd = null;
						Ext.Msg.alert("<fmt:message key='aimir.error'/>", "<fmt:message key='aimir.hems.alert.notMatchPassword'/>");
		        	}
		        } else {
		        	fp_password = null;
					fp_confirmPwd = null;
		    ...
		    buttons: [{
		      ...
		      handler: function() { 
		        ...
		        if(cancelText.length > 255) {
		        	...
		        	return;
		        } else if (rec.payType == "Mobile") {
		        	Ext.Msg.alert("<fmt:message key='aimir.warning'/>","It is not possible to cancel the charge with the \"Mobile\" type.");
		        	return;
		        } else {
		        	Ext.Msg.wait('Waiting for response.', 'Wait !');
      ...
      cmdGetRemainingCredit: function() {
    	  if(selectedMeterId != null && selectedMeterId+"" != '') {
    		  Ext.Msg.show({
	         		title: '<b>Get RemainingCredit<b/>',
	         		msg: 'Do you want to get?',
	         		buttons : Ext.MessageBox.OKCANCEL,
	         		fn : function(btn) {
	              	if(btn == 'ok') {
	              		Ext.Msg.wait('Waiting for response.', 'Wait !');
	              		$.getJSON('${ctx}/gadget/device/command/cmdGetRemainingCredit.do', {
	                    	  ...
	                    	  Ext.Msg.hide();
                              Ext.Msg.alert('Message', returnData.result);
	                      });
	              	} else {
	              		return;
	              	}
    		  ...
    		  Ext.Msg.alert("<fmt:message key='aimir.message'/>","<fmt:message key='aimir.contract.selectContract'/>");
    	  }
      }
    };
    ...
    var bind = function() {
      ...
      $("#contractForm span#setSTSToken").click(eventHandler.setSTSToken);
      $("#contractForm span#cmdTcpRetry").click(eventHandler.cmdTcpRetry);
      $("#contractForm span#remainingCredit").click(eventHandler.cmdGetRemainingCredit);
    ...
    function addTooltip2(value, metadata) {
 	...
 	// OPF-271 Send STS Token to Target
