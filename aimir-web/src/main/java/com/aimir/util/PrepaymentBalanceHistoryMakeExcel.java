...
public class PrepaymentBalanceHistoryMakeExcel {
	...
	public void writeReportExcel(List<Map<String, Object>> result, Map<String, String> msgMap, boolean isLast, String filePath, String fileName, Supplier supplier) {
		...
		try {
			...
			cell.setCellValue(msgMap.get("cancelAmount"));
			cell.setCellStyle(titleCellStyle);
			...
			for (int i = 0; i < dataCount; i++) {
				...
				cell = row.createCell(4);
				if (resultMap.get("isCanceled") != null) {
					cell.setCellValue(cdf.format(resultMap.get("chargedCredit")));
				} else {
					cell.setCellValue("");
				}
				cell.setCellStyle(dataCellStyle);
				...
				cell = row.createCell(11);
				cell.setCellValue(resultMap.get("cancelReason") == null ? "" : resultMap.get("cancelReason").toString());
				cell.setCellStyle(dataCellStyle);
			}
			// End Data
