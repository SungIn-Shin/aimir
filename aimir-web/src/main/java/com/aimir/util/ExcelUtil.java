...
public class ExcelUtil {
    ...
    public static HSSFCellStyle getStyle(HSSFWorkbook workbook, HSSFFont font, int top, int bottom, int left, int right, int grey,
        ...
        if(border == 0) {
        	...
        	borderStyle = HSSFCellStyle.BORDER_MEDIUM;
        }
...
//            style.setFillForegroundColor(IndexedColors.TAN.getIndex());
	...
	public static XSSFCellStyle getStyle(XSSFWorkbook workbook, XSSFFont font, int top, int bottom, int left,
			int right, int grey, int green, int orange, int align, int border, int cellFormat) {
		 XSSFCellStyle style = workbook.createCellStyle();
		 XSSFDataFormat format = workbook.createDataFormat();
		 
		 XSSFColor a24 = new XSSFColor(new java.awt.Color(72,175,246));
		 XSSFColor na24 = new XSSFColor(new java.awt.Color(231,231,26));
		 XSSFColor na48 = new XSSFColor(new java.awt.Color(255,154,22));
		 XSSFColor unknown = new XSSFColor(new java.awt.Color(234,107,115));
		 XSSFColor total = new XSSFColor(new java.awt.Color(96,109,246));
		 
	        style.setFont(font);
	        short borderStyle = XSSFCellStyle.BORDER_MEDIUM;
	        
	        if(border == 0) {
	        	borderStyle = XSSFCellStyle.BORDER_THIN;
	        }else if(border == 1) {
	        	borderStyle = XSSFCellStyle.BORDER_MEDIUM;
	        }
	        ...
	        if (grey == 1) { // GREY
	        	...
	        	style.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
	        	style.setFillForegroundColor(na48);
	        }else if (orange == 1) { // ORANGE 
	            style.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
	            style.setFillForegroundColor(IndexedColors.LIGHT_ORANGE.getIndex());
//	            style.setFillForegroundColor(IndexedColors.TAN.getIndex());
	        	...
	        	style.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
	            style.setFillForegroundColor(unknown);
	        }else if (orange == 3) {
	        	style.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
	        	style.setFillForegroundColor(a24);
	        }else if (orange == 4) {
	        	style.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
	        	style.setFillForegroundColor(na24);
	        }else if (orange == 5) {
				style.setFillPattern(HSSFCellStyle.SOLID_FOREGROUND);
				style.setFillForegroundColor(total);
			}
