...
import com.aimir.constants.StsConstants;
...
public class DebtEntManagerImpl implements DebtEntManager{
	...
	private Map<String, Object> vendorBalanceChargingECG(Map<String, Object> condition) {
    	...
    	try {
    		...
    		//STS타입별로 구분해야한다..(1:Suni, 2:Wasion, 3:Nuri)
    		Boolean contractSTS = contract.getIsSts()==null? false:contract.getIsSts();
    		Boolean isSTS = false;
    		String token=null;    		    		
    		if(contractSTS && meter != null && meter.getModel() != null) {
    			isSTS = true;
				...
				*/
				...
				//Type1: SuniSTS는 미터모델명으로 구분함.
    			if(modelName!=null && "OmniPower STS".equals(modelName)) {    				
    				...
    				if(token == null || token.length() != 20) {
    			        ...
    			        Map<String,String> paramMap = new HashMap<String,String>();
    		 			...
    		 			String cmd = StsConstants.SuniCMD.cmdSetSTSToken.name();
				...
				//Type2: WASION은 일단 Vendor로 구분. IhdID 여부 등으로 구분하는게 좋을것 같다.	
    			}else if(vendorName!=null && "Wasion".equals(vendorName) && !"NRAM-3405CT10".equals(modelName)) {
    				...
    				if(token == null || token.length() != 20) {
    					...
    					//비동기 내역 저장 - 기존 ECG외엔 비동기명령 사용하지 않는다. STS로그만 저장함.
    					List<Map<String,String>> paramList = new ArrayList<Map<String,String>>();
    			        Map<String,String> paramMap = new HashMap<String,String>();
    		 			paramMap.put("string", token);
    		 			paramList.add(paramMap);
	...
	public Map<String,Object> cancelTransaction(Long id, String operatorId, String reason) {
		...
		try {
			...
			if(!isCancelled) {
				...
				//STS타입별 구분 (Type1: Suni, 2:Wasion, 3:Nuri)
				Boolean contractSTS = contract.getIsSts()==null? false:contract.getIsSts();				
				
				//STS이고, 토큰이 발행되었는지 체크 "OmniPower STS".equals(meter.getModel().getName())
				if(prepayLog.getToken() != null && meter.getModel() != null && contractSTS) {
					String modelName = meter.getModel().getName();
	    			String vendorName = meter.getModel().getDeviceVendor().getName();
					...
					if(chargedCredit > 0) {
						...
						//Type2: WASION은 일단 Vendor로 구분. IhdID 여부 등으로 구분하는게 좋을것 같다.		
							...
							//Wasion은 sts number가 없는데, 일단 mdsID로 대체함.
							String DSN=meter.getMdsId();
							...
							if(cancelToken == null || cancelToken.length() != 20) {
		    					...
		    					//비동기 내역 저장 (TVWS부터는 비동기 사용하지 않는다)
		    					List<Map<String,String>> paramList = new ArrayList<Map<String,String>>();
		    			        Map<String,String> paramMap = new HashMap<String,String>();
		    		 			paramMap.put("string", cancelToken);
		    		 			paramList.add(paramMap);
...
//        Double totalAmount = ((chargedCredit == null) ? 0D : chargedCredit) + ((chargedArrears == null) ? 0D : chargedArrears);
        ...
        String cancelDate = TimeLocaleUtil.getLocaleDate(StringUtil.nullToBlank(prepaymentLogWithDebt.get("CANCELDATE")), lang, country);
        ...
        if(cancelToken != null && cancelToken.length() == 20) {
        	Formatter formatter = new Formatter();
    		formatter.format("%4s %4s %4s %4s %4s", cancelToken.substring(0, 4), cancelToken.substring(4,8), cancelToken.substring(8,12), cancelToken.substring(12,16), cancelToken.substring(16,20));
