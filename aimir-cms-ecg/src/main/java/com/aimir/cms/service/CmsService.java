...
public class CmsService {
    ...
    private void modServPoint(CMSEnt cmsEnt) throws CMSException {
        ...
        if(contracts.size() == 1 && servPoint.isExist()){
            ...
            if (servpoints != null && servpoints.size() == 1) {
                ...
                // 2018.03.13 추가됨
                _servPoint.setDigitalAddress(servPoint.getDigitalAddress());
    ...
    private void installMeter(CMSEnt cmsEnt) throws CMSException {
        ...
        if (contracts != null && contracts.size() == 1 && meters != null && meters.size() == 1) {
            ...
            contract.setMeter(meter);
            contract.setContractDate(meterEnt.getWriteDate());
            contract.setPrepayStartTime(meterEnt.getWriteDate());
    ...
    private void removeMeter(CMSEnt cmsEnt) throws CMSException {
        ...
        if(contracts.size() == 1 && servPoint.isExist()){
            ...
            if (meter != null) {
                ...
                deleteCode = codeDao.findByCondition("code", "1.2.7.2");
                if (deleteCode != null && meter.getModem() != null) {
                    meter.getModem().setModemStatus(deleteCode);
        ...
        if (custEnt != null && custEnt.getCustomerId() != null && !"".equals(custEnt.getCustomerId())) {
            ...
            if (customers != null && customers.size() == 1) {
                ...
                if (custEnt.getServPoint() != null) {
                    ...
                    // 2018.03.13 추가
                    servPoint.setDigitalAddress(custEnt.getServPoint().getDigitalAddress());
    ...
    private String getMeterModel(String cmsMeterModel) {
        ...
        else if (cmsMeterModel.equals("ML089")) { 
            modelName = "OmniPower STS";
        }
    ...
    public void meterImport( AuthCred authCred, MeterEnt meterEnt)
        ...
        if (modelName == "OmniPower STS") {

			Set<Condition> condition = new HashSet<Condition>();
			condition.add(new Condition("meterId", new Object[] { meter.getId() }, null, Restriction.EQ));
			List<Contract> contracts = contractDao.findByConditions(condition);

			if (contracts == null || contracts.size() != 1) {
				throw new CMSException(ErrorType.Error.getIntValue(), "Meter Serial [" + mdsId + "]'s Contract Information is empty or 2媛??댁긽. ");
			}
